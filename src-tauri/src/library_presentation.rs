...
const COVER_BYTES_CACHE_MAX_BYTES: u64 = 256 * 1024 * 1024;
...
static NEXT_CACHE_REPLACEMENT: AtomicU64 = AtomicU64::new(0);
#[cfg(target_os = "macos")]
...
struct PresentationMetadata {
    ...
    complete_identity_agreement: bool,
    source: Option<String>,
    provider_id: Option<String>,
    legacy_provider_id: Option<String>,
    legacy_display_code: Option<String>,
...
fn retain_identity_conflict(
    ...
    let protected = active_cover_request_keys(context);
    merge_cache_entry_with_protected(cache_path, conflict_cache_entry(authority), &protected)?;
    context
        .identity_conflicts
...
fn exact_legacy_content_id(value: &str) -> Option<String> {
    (!value.is_empty()
        && value.len() <= 64
        && value
            .bytes()
...
fn exact_product_code_value(value: &str, identity: &str) -> bool {
    value.trim() == value
        && !value.bytes().any(|byte| byte.is_ascii_control())
        && crate::vr_torrent::canonical_product_code(value).as_deref() == Some(identity)
}

fn exact_legacy_display_code(
    value: Option<&JsonValue>,
    authority: &LibraryItemAuthority,
) -> Option<String> {
    let value = match value {
        Some(JsonValue::String(value)) => value,
        None | Some(JsonValue::Null) | Some(_) => return None,
    };
    (crate::vr_torrent::product_code_display_form(value).as_deref() == Some(value)
        && crate::vr_torrent::canonical_product_code(value).as_deref()
            == Some(&authority.product_identity))
    .then(|| value.to_owned())
}
...
fn exact_legacy_cover(
    document: &str,
    authority: &LibraryItemAuthority,
) -> Result<LegacyItemProof, ProviderRequestError> {
    ...
    let content_id = exact_legacy_content_id(content_id).ok_or(ProviderRequestError::Provider)?;
    let display_code = exact_legacy_display_code(root.get("dvd_id"), authority)
        .ok_or(ProviderRequestError::Provider)?;
    let images = match root.get("images") {
        None | Some(JsonValue::Null) => {
            return Ok(LegacyItemProof {
                content_id,
                display_code,
                cover_url: None,
            })
    ...
    for key in ["large2", "large"] {
        let url = match jacket.get(key) {
            ...
            Some(JsonValue::String(url))
                if exact_legacy_image_content_id(&content_id, url).is_some() =>
            {
                url
            }
            Some(_) => return Err(ProviderRequestError::Provider),
...
fn metadata_from_legacy(
    ...
    let proof = exact_legacy_cover(document, authority)?;
    let Some(JsonValue::Object(root)) = JsonParser::new(document).parse() else {
        return Err(ProviderRequestError::Provider);
    };
    ...
    let metadata = PresentationMetadata {
        verified_identity: Some(VerifiedDisplayIdentity {
            provider: "r18.dev",
            provider_id: proof.content_id.clone(),
            display_code: proof.display_code.clone(),
        }),
        identity_conflict: false,
        complete_identity_agreement: false,
        source: Some("r18.dev".to_owned()),
        provider_id: Some(proof.content_id.clone()),
        legacy_provider_id: Some(proof.content_id),
        legacy_display_code: Some(proof.display_code.clone()),
        display_code: Some(proof.display_code),
        title,
        date,
        runtime,
        cast,
    };
    Ok(metadata.is_ready().then_some(metadata))
}
...
fn exact_legacy_image_content_id(provider_id: &str, url: &str) -> Option<String> {
    if !valid_legacy_cover_url(url) {
        return None;
    }
    let content_id = exact_legacy_content_id(provider_id)?;
...
fn cover_source_valid(
    ...
    !source.provider_id.is_empty()
        ...
        && match source.provider {
            ...
            "r18.dev" => {
                crate::vr_torrent::canonical_product_code(&source.display_code)
                    == crate::vr_torrent::canonical_product_code(code)
                    && exact_legacy_image_content_id(&source.provider_id, &source.url).is_some()
            }
            _ => false,
        }
}
...
fn verified_display_identity_valid(
    ...
    crate::vr_torrent::product_code_display_form(&identity.display_code).as_deref()
        ...
        && match identity.provider {
            ...
            "r18.dev" => exact_legacy_content_id(&identity.provider_id).is_some(),
            _ => false,
        }
}
...
fn verified_identity_from_cover(source: &CoverSource) -> Option<VerifiedDisplayIdentity> {
    matches!(source.provider, "JavDB" | "FANZA" | "r18.dev").then(|| VerifiedDisplayIdentity {
...
fn reconcile_verified_identities(
    ...
    if current.provider == "JavDB" {
        ...
        Ok(Some(candidate))
    } else if current.provider == "FANZA" {
        Ok(Some(current))
    } else if candidate.provider == "FANZA" {
        Ok(Some(candidate))
    } else {
        Ok(Some(current))
    }
...
fn cover_identity_consistent(
    ...
    match source.provider {
        ...
        "r18.dev" => identity.is_some_and(|identity| {
            identity.display_code == source.display_code
                && (identity.provider != "r18.dev" || identity.provider_id == source.provider_id)
        }),
        _ => false,
    }
}

fn metadata_source_valid(source: &str) -> bool {
    matches!(
        source,
        "JavDB"
            | "JavDB + r18.dev"
            | "JavDB + JavDatabase"
            | "JavDB + r18.dev + JavDatabase"
            | "r18.dev"
            | "r18.dev + JavDatabase"
            | "JavDatabase"
    )
...
fn metadata_identity_consistent(
    ...
    let Some(source) = metadata.source.as_deref() else {
        return false;
    };
    if !metadata_source_valid(source) {
        return false;
    }
    let includes_legacy = source.split(" + ").any(|provider| provider == "r18.dev");
    let Some(product_identity) = crate::vr_torrent::canonical_product_code(code) else {
        return false;
    };
    if includes_legacy
        && !(metadata
            .legacy_provider_id
            .as_ref()
            .is_some_and(|provider_id| exact_legacy_content_id(provider_id).is_some())
            && metadata
                .legacy_display_code
                .as_ref()
                .is_some_and(|code| exact_product_code_value(code, &product_identity)))
    {
        return false;
    }
    if includes_legacy
        && identity.is_some_and(|identity| {
            metadata.legacy_display_code.as_deref() != Some(identity.display_code.as_str())
                || (identity.provider == "r18.dev"
                    && metadata.legacy_provider_id.as_deref()
                        != Some(identity.provider_id.as_str()))
        })
    {
        return false;
    }
    if !includes_legacy
        && (metadata.legacy_provider_id.is_some() || metadata.legacy_display_code.is_some())
    {
        return false;
    }
    if source.starts_with("JavDB") {
        identity.is_some_and(|identity| {
            identity.provider == "JavDB"
                && metadata.provider_id.as_deref() == Some(identity.provider_id.as_str())
        })
    } else if source.starts_with("r18.dev") {
        metadata.provider_id.as_ref().is_some_and(|provider_id| {
            exact_legacy_content_id(provider_id).is_some()
                && metadata.legacy_provider_id.as_deref() == Some(provider_id)
        })
    } else if source == "JavDatabase" {
        metadata
            .provider_id
            .as_ref()
            .is_some_and(|provider_id| exact_product_code_value(provider_id, &product_identity))
    } else {
        false
    }
}
...
fn resolve_cover_with_current_identity(
    authority: (&LibraryItemAuthority, Option<VerifiedDisplayIdentity>, bool),
    ...
    let (authority, current_identity, require_complete_identity_agreement) = authority;
    ...
    match fetch_exact_javdb_item(authority, &mut javdb_document) {
        Ok(Some(item)) => {
            ...
            let mut javdb_metadata = metadata_from_javdb(&item);
            attach_verified_identity(&mut javdb_metadata, verified_identity.clone());
            metadata = Some(javdb_metadata);
    ...
    match fanza_catalog::fetch_exact_library_item_with(
        ...
        Ok(Some(item)) => {
            ...
            verified_identity = match reconcile_verified_identities(
                verified_identity
                    .clone()
                    .or_else(|| current_identity.clone()),
                Some(fanza_identity),
            ) {
                Ok(identity) => identity,
                Err(()) => {
    ...
    match legacy_document(&legacy_url(&authority.product_identity)) {
        Ok(document) => {
            let proof = match exact_legacy_cover(&document, authority) {
                Ok(proof) => proof,
                Err(_) => {
                    transient = true;
                    ...
                    return (None, metadata, transient);
                }
            };
            let legacy_identity = VerifiedDisplayIdentity {
                provider: "r18.dev",
                provider_id: proof.content_id.clone(),
                display_code: proof.display_code.clone(),
            };
            ...
            match metadata_from_legacy(&document, authority) {
                Ok(Some(legacy_metadata)) if metadata.is_none() => {
...
fn resolve_cover_with(
    ...
    javdb_image: impl FnMut(&str) -> Result<Vec<u8>, ProviderRequestError>,
    fanza_document: impl FnOnce(&str) -> Result<String, ProviderRequestError>,
    fanza_image: impl FnOnce(&str) -> Result<Vec<u8>, ProviderRequestError>,
        ...
        Ok(document) => match metadata_from_legacy(&document, authority) {
            Ok(Some(metadata)) => {
                if let Some(current) = &mut accepted {
                    let reconciled = reconcile_verified_identities(
                        current.verified_identity.clone(),
                        metadata.verified_identity.clone(),
                    );
                    let Ok(identity) = reconciled else {
                        return Ok(Some(PresentationMetadata {
                            identity_conflict: true,
                            ..PresentationMetadata::default()
                        }));
                    };
                    ...
                    current.legacy_display_code = metadata.legacy_display_code;
                    current.source = Some("JavDB + r18.dev".to_owned());
                    attach_verified_identity(current, identity);
                } else {
...
fn read_cache(path: &Path) -> Result<Vec<CacheEntry>, ()> {
    ...
    for line in lines {
        ...
        let metadata = PresentationMetadata {
            ...
            legacy_provider_id: optional_text(fields[18]),
            legacy_display_code: optional_text(fields[24]),
            display_code: optional_text(fields[19]),
            title: optional_text(fields[20]),
            date: optional_text(fields[21]),
            runtime: optional_text(fields[22]),
            cast: fields[23]
                .split(',')
...
fn write_cache(path: &Path, entries: &[CacheEntry]) -> Result<(), ()> {
    ...
    for entry in entries {
        ...
        let fields = [
            ...
            encode_text(entry.metadata.legacy_provider_id.as_deref().unwrap_or("")),
            ...
            entry
                ...
                .collect::<Vec<_>>()
                .join(","),
            encode_text(entry.metadata.legacy_display_code.as_deref().unwrap_or("")),
        ];
        writeln!(file, "{}", fields.join("\t")).map_err(|_| ())?;
    }
    file.sync_all().map_err(|_| ())?;
    if file.metadata().map_err(|_| ())?.len() > CACHE_MAX_BYTES {
        drop(file);
        remove_cover_bytes_path(&replacement);
        return Err(());
    }
    drop(file);
...
fn cover_bytes_directory(cache_path: &Path) -> Result<PathBuf, ()> {
    let file_name = cache_path
        .file_name()
        .and_then(|value| value.to_str())
        .ok_or(())?;
    Ok(cache_path.with_file_name(format!("{file_name}.covers")))
}
...
fn cover_bytes_path(
    ...
    let proof = format!(
        ...
        source.display_code,
        source.url,
        source.aspect_ratio.to_bits(),
    );
    Ok(cover_bytes_directory(cache_path)?.join(format!("{}.cover", hex_sha1(proof.as_bytes()))))
}

fn remove_cover_bytes_path(path: &Path) {
    if fs::symlink_metadata(path).is_ok() {
        let _ = fs::remove_file(path);
    }
}

fn remove_cached_cover_bytes(
    cache_path: &Path,
    authority: &LibraryItemAuthority,
    source: &CoverSource,
) {
    if let Ok(path) = cover_bytes_path(cache_path, authority, source) {
        remove_cover_bytes_path(&path);
    }
}

fn retained_cover_byte_paths(
    cache_path: &Path,
    entries: &[CacheEntry],
    now: u64,
) -> HashSet<PathBuf> {
    entries
        .iter()
        .filter(|entry| {
            entry.cover_state == "ready"
                && entry.cover_saved_at > 0
                && entry.cover_saved_at <= now
                && now.saturating_sub(entry.cover_saved_at) <= COVER_TTL_SECONDS
        })
        .filter_map(|entry| {
            let source = entry.cover.as_ref()?;
            let authority = LibraryItemAuthority {
                category: entry.category,
                identity: entry.identity.clone(),
                code: entry.code.clone(),
                product_identity: crate::vr_torrent::canonical_product_code(&entry.code)?,
            };
            cover_bytes_path(cache_path, &authority, source).ok()
        })
        .collect()
}

fn clean_cover_bytes_cache(
    cache_path: &Path,
    entries: &[CacheEntry],
    now: u64,
) -> Result<(usize, u64), ()> {
    let directory = cover_bytes_directory(cache_path)?;
    ...
    let retained = retained_cover_byte_paths(cache_path, entries, now);
    let mut count = 0;
    let mut bytes = 0_u64;
    for child in fs::read_dir(&directory).map_err(|_| ())? {
        let child = child.map_err(|_| ())?;
        let path = child.path();
        let child_metadata = fs::symlink_metadata(&path).map_err(|_| ())?;
        let keep = retained.contains(&path)
            && !child_metadata.file_type().is_symlink()
            && child_metadata.is_file()
            && child_metadata.len() <= COVER_MAX_BYTES as u64;
        if keep {
            count += 1;
...
fn read_cached_cover_bytes(
    cache_path: &Path,
    authority: &LibraryItemAuthority,
    source: &CoverSource,
) -> Result<Vec<u8>, ()> {
    let path = cover_bytes_path(cache_path, authority, source)?;
    let metadata = fs::symlink_metadata(&path).map_err(|_| ())?;
    if metadata.file_type().is_symlink()
        || !metadata.is_file()
        || !(COVER_MIN_BYTES as u64..=COVER_MAX_BYTES as u64).contains(&metadata.len())
    {
        remove_cover_bytes_path(&path);
        return Err(());
    }
    ...
    let (bytes, ratio) = match validate_cover(bytes) {
    ...
    if ratio.to_bits() != source.aspect_ratio.to_bits() {
        remove_cover_bytes_path(&path);
        return Err(());
    }
    Ok(bytes)
...
fn write_cached_cover_bytes(
    ...
    entries: &[CacheEntry],
    now: u64,
    protected: &HashSet<(LibraryPresentationCategory, String)>,
) -> Result<(), ()> {
    write_cached_cover_bytes_with_limits(
        cache_path,
        authority,
        source,
        bytes,
        ...
        now,
        protected,
        COVER_BYTES_CACHE_MAX_FILES,
        COVER_BYTES_CACHE_MAX_BYTES,
    )
}

#[allow(clippy::too_many_arguments)]
fn write_cached_cover_bytes_with_limits(
    cache_path: &Path,
    authority: &LibraryItemAuthority,
    source: &CoverSource,
    bytes: &[u8],
    ...
    let (_, ratio) = validate_cover(bytes.to_vec()).map_err(|_| ())?;
    if ratio.to_bits() != source.aspect_ratio.to_bits() {
        return Err(());
    }
    let directory = cover_bytes_directory(cache_path)?;
    ...
    let path = cover_bytes_path(cache_path, authority, source)?;
    if read_cached_cover_bytes(cache_path, authority, source).is_ok() {
        return Ok(());
    }
    let mut bounded_entries = entries.to_vec();
    let (mut count, mut retained_bytes) =
        clean_cover_bytes_cache(cache_path, &bounded_entries, now)?;
    while count.saturating_add(1) > maximum_files
        || retained_bytes.saturating_add(bytes.len() as u64) > maximum_bytes
    {
        let candidate = bounded_entries
            .iter()
            .enumerate()
            .filter(|(_, entry)| {
                entry.cover_state == "ready"
                    && entry.identity != authority.identity
                    && !protected.contains(&(entry.category, entry.code.clone()))
            })
            .filter_map(|(index, entry)| {
                let source = entry.cover.as_ref()?;
                let candidate_authority = LibraryItemAuthority {
                    category: entry.category,
                    identity: entry.identity.clone(),
                    code: entry.code.clone(),
                    product_identity: crate::vr_torrent::canonical_product_code(&entry.code)?,
                };
                let path = cover_bytes_path(cache_path, &candidate_authority, source).ok()?;
                fs::symlink_metadata(&path).ok().map(|metadata| {
                    (
                        index,
                        entry.cover_saved_at,
                        entry.identity.clone(),
                        path,
                        metadata.len(),
                    )
        ...
        let Some((index, _, _, path, length)) = candidate else {
            return Err(());
        };
        remove_cover_bytes_path(&path);
        bounded_entries[index].cover_saved_at = 0;
        bounded_entries[index].cover_state = "missing";
        bounded_entries[index].cover = None;
        count = count.saturating_sub(1);
        retained_bytes = retained_bytes.saturating_sub(length);
    }
    if bounded_entries != entries {
        write_cache(cache_path, &bounded_entries)?;
    }
    let replacement_id = NEXT_CACHE_REPLACEMENT.fetch_add(1, Ordering::Relaxed);
...
fn cache_entry_recency(entry: &CacheEntry) -> u64 {
    entry
        .identity_saved_at
        .max(entry.cover_saved_at)
        .max(entry.metadata_saved_at)
}

fn merge_cache_entry_with_protected(
    path: &Path,
    entry: CacheEntry,
    protected: &HashSet<(LibraryPresentationCategory, String)>,
) -> Result<(), ()> {
    ...
    let replaced = entries
        .iter()
        .find(|current| current.identity == entry.identity)
        .cloned();
        ...
        let candidate = entries
            ...
            .filter(|(_, current)| {
                current.identity != entry.identity
                    && !protected.contains(&(current.category, current.code.clone()))
            })
            .min_by(|(_, left), (_, right)| {
                (cache_entry_recency(left), &left.identity)
                    .cmp(&(cache_entry_recency(right), &right.identity))
            })
            .map(|(index, _)| index)
            .ok_or(())?;
        evicted.push(entries.remove(candidate));
    }
    entries.sort_by(|left, right| left.identity.cmp(&right.identity));
    write_cache(path, &entries)?;
    if let Some(replaced) = replaced {
        if let Some(source) = &replaced.cover {
            let retained = entry
                .cover
                .as_ref()
            ...
            if !retained {
                let authority = LibraryItemAuthority {
                    ...
                    identity: replaced.identity,
                    code: replaced.code.clone(),
                    product_identity: crate::vr_torrent::canonical_product_code(&replaced.code)
                        .unwrap_or_default(),
                };
                remove_cached_cover_bytes(path, &authority, source);
            }
        }
    }
    for evicted in evicted {
        if let Some(source) = &evicted.cover {
            if let Some(product_identity) = crate::vr_torrent::canonical_product_code(&evicted.code)
            {
                remove_cached_cover_bytes(
                    path,
...
fn merge_cache_entry(path: &Path, entry: CacheEntry) -> Result<(), ()> {
    merge_cache_entry_with_protected(path, entry, &HashSet::new())
}

fn active_cover_request_keys(
    context: &LibraryPresentationContext,
) -> HashSet<(LibraryPresentationCategory, String)> {
    context
        .cover_requests
        .iter()
...
fn remove_cache_entry(path: &Path, identity: &str) -> Result<(), ()> {
    ...
    if let Some(entry) = removed {
        if let Some(source) = &entry.cover {
            if let Some(product_identity) = crate::vr_torrent::canonical_product_code(&entry.code) {
                remove_cached_cover_bytes(
                    path,
                    &LibraryItemAuthority {
                        category: entry.category,
...
fn expected_cover_authority_id(
    ...
    if source.provider == "r18.dev" {
        let image_content_id = exact_legacy_image_content_id(&source.provider_id, &source.url)?;
...
fn resolve_cover_at_with(
    ...
    resolve_cover_at_with_request(state, cache_path, authority, 0, now, is_current, |_| {
        resolve()
    })
}
...
fn resolve_cover_at_with_request(
    ...
    now: u64,
    is_current: impl Fn() -> bool,
    resolve: impl FnOnce(Option<VerifiedDisplayIdentity>) -> CoverResolution,
) -> Result<Vec<String>, &'static str> {
    let mut cache = {
    ...
    clean_cover_bytes_cache(cache_path, &cache, now).map_err(|_| LIBRARY_PRESENTATION_FAILED)?;
    ...
    if let Some(entry_index) = cache.iter().position(|entry| {
        cache_entry_matches_authority(entry, authority)
            && entry.cover_state != "conflict"
            && entry.cover_saved_at > 0
            && entry.cover_saved_at <= now
        ...
        let entry = cache[entry_index].clone();
        ...
        let source = entry.cover.as_ref().ok_or(LIBRARY_PRESENTATION_FAILED)?;
        match read_cached_cover_bytes(cache_path, authority, source) {
            Ok(bytes) => {
                return cover_response(
                    state,
                    authority,
            ...
            Err(()) => {
                remove_cached_cover_bytes(cache_path, authority, source);
                cache[entry_index].cover_saved_at = 0;
                cache[entry_index].cover_state = "missing";
                cache[entry_index].cover = None;
                write_cache(cache_path, &cache).map_err(|_| LIBRARY_PRESENTATION_FAILED)?;
            }
        }
    }

    let (cover, mut metadata_seed, transient) = resolve(
        cached_identity
            .as_ref()
    ...
    let fresh_identity = match reconcile_verified_identities(
        metadata_seed
            .as_ref()
    ...
    let has_fresh_identity = fresh_identity.is_some();
    ...
    let (identity_saved_at, verified_identity) = match resolved_identity {
        Some(identity) if has_fresh_identity => (now, Some(identity)),
        Some(identity) => (
            cached_identity
                .as_ref()
                .map_or(now, |(saved_at, _)| *saved_at),
            Some(identity),
        ),
        None if transient => (0, None),
        None => (0, None),
    };
    ...
    let complete_identity_agreement = metadata_seed
        .as_ref()
    ...
    if cover_state != "unavailable" || verified_identity.is_some() {
        ...
        let protected = active_cover_request_keys(&_guard);
        merge_cache_entry_with_protected(cache_path, entry, &protected)
            .map_err(|_| LIBRARY_PRESENTATION_FAILED)?;
    }
...
pub(crate) fn resolve_metadata(
    ...
    resolve_metadata_at_with(
        ...
        || {
            ...
            let (identity_conflict, seed, verified_identity) = {
                let mut context = state.0.lock().map_err(|_| ProviderRequestError::Provider)?;
                (
                    context.identity_conflicts.contains(&key),
...
fn metadata_response(
    ...
    response.push(metadata.legacy_provider_id.clone().unwrap_or_default());
    response.push(metadata.legacy_display_code.clone().unwrap_or_default());
    response
}
...
fn fetch_cover_with(
    ...
    .map(|(bytes, _)| bytes)
    .map_err(|_| LIBRARY_PRESENTATION_FAILED)?;
    let guard = state.0.lock().map_err(|_| LIBRARY_PRESENTATION_FAILED)?;
    let entries =
        load_cache_for_authority(cache_path, authority).map_err(|_| LIBRARY_PRESENTATION_FAILED)?;
    if entries.iter().any(|entry| {
        cache_entry_matches_authority(entry, authority)
            && entry.cover_state == "ready"
            && entry.cover.as_ref() == Some(&cover.source)
    }) {
        let protected = active_cover_request_keys(&guard);
        write_cached_cover_bytes(
            cache_path,
            authority,
            &cover.source,
            &bytes,
...
pub(crate) fn begin_cover_request(
    ...
    context
        .identity_conflicts
        .retain(|seed| seed.category != category || seed.code != code);
...
mod tests {
    ...
    fn write_cover_bytes_for_cached_entry(path: &Path, authority: &LibraryItemAuthority, now: u64) {
        let entries = read_cache(path).expect("the cache proof must be valid");
        let source = entries
            .iter()
            .find(|entry| cache_entry_matches_authority(entry, authority))
            .and_then(|entry| entry.cover.as_ref())
            .expect("the cache entry must contain a cover");
        write_cached_cover_bytes(
            path,
            authority,
            source,
        ...
        .expect("the validated local cover bytes must be stored");
    }
    ...
    fn fanza_exact_identity_survives_a_genuine_missing_package_image() {
        for (category, code, identity, content_id, content_type, display_code) in [
            ...
            let (cover, evidence, transient) = resolve_cover_with(
                ...
                |_| {
                    Ok(format!(
                        r#"{{"content_id":"{content_id}","dvd_id":"{display_code}","images":null}}"#
                    ))
                },
    ...
    fn canonical_only_cross_provider_agreement_is_a_durable_conflict() {
        for (index, authority, javdb_display, content_id, content_type, fanza_display) in [
            ...
            begin_cover_request(&state, authority.category, &authority.code, 1)
                .expect("the conflicting cover request must begin");
            let response = resolve_cover_at_with_request(
                &state,
                &fixture.path,
                &authority,
                1,
                ...
                |current_identity| {
                    resolve_cover_with_current_identity(
                        (&authority, current_identity, false),
                        |url| {
                            Ok(if url.contains("search") {
            ...
            for attempt in 0..2 {
                let metadata_response =
                    resolve_metadata(&state, &fixture.path, &authority, 1, || true)
                        .expect("metadata Retry must preserve the retained conflict");
                assert_eq!(metadata_response[2], "unavailable", "attempt {attempt}");
            ...
            let recovery_fanza_calls = Cell::new(0);
            let recovered = resolve_cover_at_with_request(
                ...
                |current_identity| {
                    resolve_cover_with_current_identity(
                        ...
                        |url| {
                            Ok(if url.contains("search") {
                                javdb_listing(fanza_display)
                            } else {
                                javdb_detail(fanza_display, authority.category)
                            })
                        },
                        |_| Ok(jpeg(600, 800)),
                        |_| {
                            recovery_fanza_calls.set(recovery_fanza_calls.get() + 1);
                            Ok(format!(
                                r#"{{"data":{{"c0":{{"id":"{content_id}","contentType":"{content_type}","title":"Exact","packageImage":null}}}}}}"#
                            ))
                        },
                        |_| panic!("the agreed null FANZA cover has no image request"),
                        |_| {
                            Ok(format!(
                                r#"{{"content_id":"{}","images":null}}"#,
                                authority.code
                            ))
                        },
                        |_| panic!("an absent legacy cover has no image request"),
                    )
                },
            )
            .expect("exact agreement must recover only through a full cover retry");
            assert_eq!(recovered[2], "ready");
            assert_eq!(recovered[3], "JavDB");
            assert_eq!(&recovered[8..], ["JavDB", "item", fanza_display]);
            assert_eq!(recovery_fanza_calls.get(), 1);
            let recovered_metadata =
                resolve_metadata(&state, &fixture.path, &authority, 2, || true)
                    .expect("agreed metadata must become current after full retry");
            assert_eq!(recovered_metadata[2], "automatic");
            assert_eq!(recovered_metadata[3], "current");
            assert!(!state
                .0
                .lock()
            ...
            fetch_cover_with(&state, &fixture.path, &authority, &recovered[6], |_| {
                panic!("the recovered bytes must already be retained")
            })
            .expect("the recovered displayed cover must become a local cache hit");
            ...
            .expect("restart must restore only the later agreed identity");
    ...
    #[test]
    fn fresh_provider_identity_reconciles_with_current_persistent_identity() {
        let scenarios = [
            (
                "same-provider-item-cawb",
                LibraryPresentationCategory::Adult,
                "CAWB-1",
                verified_javdb("olditem", "CAWB-001"),
                verified_javdb("newitem", "CAWB-001"),
                None,
            ),
            (
                "same-provider-display-cawb",
                LibraryPresentationCategory::Adult,
                "CAWB-1",
                verified_javdb("item", "CAWB-001"),
                verified_javdb("item", "CAWB-0001"),
                None,
            ),
            (
                "cross-provider-display-cawb",
                LibraryPresentationCategory::Adult,
                "CAWB-1",
            ...
            (
                "cross-provider-display-3dsvr",
                LibraryPresentationCategory::Vr,
                "3DSVR-01871",
                verified_fanza("13dsvr01871", "3DSVR-01871"),
                verified_javdb("item", "3DSVR-1871"),
                None,
            ),
            (
                "exact-agreement-cawb",
                LibraryPresentationCategory::Adult,
                "CAWB-1",
                verified_fanza("cawb00001", "CAWB-001"),
                verified_javdb("item", "CAWB-001"),
                Some(verified_javdb("item", "CAWB-001")),
            ),
            (
                "exact-agreement-3dsvr",
                LibraryPresentationCategory::Vr,
                "3DSVR-01871",
                verified_fanza("13dsvr01871", "3DSVR-01871"),
                verified_javdb("item", "3DSVR-01871"),
                Some(verified_javdb("item", "3DSVR-01871")),
            ),
        ];
        for (index, (label, category, code, current, candidate, expected)) in
            scenarios.into_iter().enumerate()
        {
            let fixture = CacheFixture::new(label);
            let authority = LibraryItemAuthority {
                category,
                identity: format!("8{index:039x}"),
                code: code.to_owned(),
                product_identity: crate::vr_torrent::canonical_product_code(code)
                    .expect("fixture code must be canonicalizable"),
            };
            write_cache(
                &fixture.path,
                &[CacheEntry {
                    ...
                    code: code.to_owned(),
                    identity_saved_at: 90,
                    verified_identity: Some(current.clone()),
                    cover_saved_at: 0,
                    cover_state: "missing",
                    cover: None,
                    metadata_saved_at: 0,
            ...
            .expect("the current identity fixture must be durable");
            let state = LibraryPresentationState::default();
            begin_cover_request(&state, category, code, 1)
                .expect("the fresh provider request must begin");
            let response = resolve_cover_at_with_request(
                &state,
                &fixture.path,
                &authority,
                ...
                || true,
                |received_current| {
                    assert_eq!(received_current, Some(current.clone()));
                    (None, Some(identity_evidence(candidate.clone())), false)
                },
            )
            .expect("identity reconciliation must return a current response");
            match expected {
                Some(expected) => {
                    ...
                    assert_eq!(
                        ...
                        [
                            ...
                            expected.display_code.as_str(),
                        ]
                    );
                    let cache = read_cache(&fixture.path)
                        .expect("the agreed identity cache must remain readable");
                    assert_eq!(cache.len(), 1);
                    assert_eq!(cache[0].verified_identity, Some(expected));
                }
                None => {
                    assert_eq!(response[2], "unavailable");
                    assert_eq!(&response[8..], ["", "", ""]);
                    ...
                    assert!(state
                        .0
                        .lock()
                        .expect("conflict state must remain readable")
                        .identity_conflicts
                        .contains(&metadata_seed_key(&authority, 1)));
                    let restarted_provider = Cell::new(false);
                    let restarted = resolve_cover_at_with(
    ...
    fn failed_fanza_row_contributes_nothing_and_later_exact_source_can_succeed() {
        ...
        for document in [
            ...
            let (cover, metadata, transient) = resolve_cover_with(
                ...
                |_| {
                    Ok(r#"{"content_id":"cawb00001","dvd_id":"CAWB-1","title_ja":"Legacy title","images":{"jacket_image":{"large":"https://pics.dmm.co.jp/digital/video/cawb00001/cawb00001pl.jpg"}}}"#.to_owned())
                },
                |_| Ok(jpeg(600, 800)),
            );
            let source = cover
    ...
    fn failed_javdb_or_legacy_cover_data_is_unavailable_and_never_cached_as_a_miss() {
        ...
        let response = resolve_cover_at_with(
            ...
            || {
                resolve_cover_with(
                    ...
                    |_| Ok(r#"{"content_id":"cawb00001","dvd_id":"CAWB-1","images":null}"#.to_owned()),
        ...
        assert_eq!(response[2], "unavailable");
        assert_eq!(javdb_images.get(), 0);
        let retained = read_cache(&javdb_fixture.path)
            .expect("a separately verified legacy display identity may remain");
        assert_eq!(retained.len(), 1);
        assert_eq!(retained[0].cover_saved_at, 0);
            ...
            let response = resolve_cover_at_with(
                ...
                || {
                    resolve_cover_with(
                        ...
                        |_| {
                            Ok(format!(
                                r#"{{"content_id":"cawb00001","dvd_id":"CAWB-1",{legacy_cover}}}"#
                            ))
                        },
                        |_| {
    ...
    #[test]
    fn legacy_cover_and_metadata_proofs_require_the_exact_current_content_identity() {
        let authority = LibraryItemAuthority {
            category: LibraryPresentationCategory::Adult,
            identity: "6".repeat(40),
            code: "CAWB-1".to_owned(),
        ...
        let exact_document = format!(
            r#"{{"content_id":"cawb00001","dvd_id":"CAWB-1","title_ja":"Exact legacy title","images":{{"jacket_image":{{"large":"{exact_url}"}}}}}}"#
        );
        let proof = exact_legacy_cover(&exact_document, &authority)
            .expect("the exact legacy document must establish its current content identity");
        assert_eq!(proof.content_id, "cawb00001");
        assert_eq!(proof.display_code, "CAWB-1");
        assert_eq!(proof.cover_url.as_deref(), Some(exact_url));
        ...
        assert_eq!(metadata.provider_id.as_deref(), Some("cawb00001"));
        assert_eq!(metadata.legacy_provider_id.as_deref(), Some("cawb00001"));
        assert_eq!(metadata.legacy_display_code.as_deref(), Some("CAWB-1"));

        let crossed_url = "https://pics.dmm.co.jp/digital/video/cawb00002/cawb00002pl.jpg";
        let crossed_document = format!(
            r#"{{"content_id":"cawb00001","dvd_id":"CAWB-1","title_ja":"Crossed legacy title","images":{{"jacket_image":{{"large":"{crossed_url}"}}}}}}"#
        );
        assert_eq!(
        ...
        for wrong_id in ["CAWB-2", "ADLT-1", " CAWB-1", "CAWB-1\n"] {
            let document = exact_document
                .replace(r#""dvd_id":"CAWB-1""#, &format!(r#""dvd_id":"{wrong_id}""#));
            assert_eq!(
        ...
        for missing_identity in [
            exact_document.replace(r#","dvd_id":"CAWB-1""#, ""),
            exact_document.replace(r#""dvd_id":"CAWB-1""#, r#""dvd_id":null"#),
        ] {
            assert_eq!(
                exact_legacy_cover(&missing_identity, &authority),
                Err(ProviderRequestError::Provider)
            );
            assert_eq!(
                metadata_from_legacy(&missing_identity, &authority),
                Err(ProviderRequestError::Provider)
            );
        }
        ...
        let metadata = resolve_metadata_with(
            &authority,
            |_| Err(ProviderRequestError::SourceUnavailable),
            |url| {
                if url.starts_with("https://r18.dev/") {
                    Ok(r#"{"content_id":"cawb00002","dvd_id":"CAWB-2","title_ja":"Crossed title"}"#.to_owned())
                } else {
                    Err(ProviderRequestError::SourceUnavailable)
                }
            },
        ...
        let unavailable = resolve_cover_at_with(
            ...
            || {
                resolve_cover_with(
                    ...
                    |_| {
                        Ok(r#"{"content_id":"cawb00001","dvd_id":"CAWB-1","title_ja":"Crossed","images":{"jacket_image":{"large":"https://pics.dmm.co.jp/digital/video/cawb00002/cawb00002pl.jpg"}}}"#.to_owned())
                    },
                    |_| {
    ...
    #[test]
    fn missing_legacy_display_identity_creates_no_proof_and_fresh_retry_can_recover() {
        let authority = LibraryItemAuthority {
            category: LibraryPresentationCategory::Adult,
            identity: "d".repeat(40),
            code: "CAWB-1".to_owned(),
        ...
        for (index, legacy_identity) in [(0, String::new()), (1, r#","dvd_id":null"#.to_owned())] {
            let fixture = CacheFixture::new(&format!("missing-legacy-identity-{index}"));
            let state = LibraryPresentationState::default();
            let legacy_images = Cell::new(0);
            let document = format!(
                r#"{{"content_id":"cawb00001"{legacy_identity},"title_ja":"Unproven","images":{{"jacket_image":{{"large":"https://pics.dmm.co.jp/digital/video/cawb00001/cawb00001pl.jpg"}}}}}}"#
            );
            ...
            let unavailable = resolve_cover_at_with(
                ...
                || {
                    resolve_cover_with(
                        &authority,
                        |_| Ok(r#"{"success":1,"data":{"movies":[]}}"#.to_owned()),
                        |_| panic!("a genuine JavDB no-match has no image request"),
                        |_| Ok(r#"{"data":{"c0":null}}"#.to_owned()),
                        |_| panic!("a genuine FANZA no-match has no image request"),
                        |_| Ok(document.clone()),
                        |_| {
                            legacy_images.set(legacy_images.get() + 1);
                            Ok(jpeg(600, 800))
            ...
            .expect("missing provider identity must be a current unavailable result");
            assert_eq!(unavailable[2], "unavailable");
            assert_eq!(&unavailable[3..7], ["", "", "", ""]);
            assert_eq!(&unavailable[8..], ["", "", ""]);
            assert_eq!(legacy_images.get(), 0);
            ...
            let retry = resolve_cover_at_with(
                ...
                || {
                    resolve_cover_with(
                        ...
                        |_| Ok(jpeg(600, 800)),
                        |_| panic!("legacy must not run after current FANZA recovery"),
                        |_| panic!("legacy image must not run after current FANZA recovery"),
                    )
                },
            )
            .expect("a fresh full retry must recover through the current provider order");
            assert_eq!(&retry[2..6], ["ready", "FANZA", "cawb00001", "CAWB-001"]);
        }
    }
            ...
            let response = resolve_cover_at_with(
                ...
                || {
                    resolve_cover_with(
                        ...
                        |_| {
                            Ok(r#"{"content_id":"cawb00001","dvd_id":"CAWB-001"$COVER}"#
    ...
    #[test]
    fn legacy_proof_uses_the_response_content_id_not_fanza_family_mapping() {
        for (category, code, identity, content_id, dvd_value) in [
            (
                LibraryPresentationCategory::Vr,
                "3DSVR-01871",
                "3DSVR-1871",
            ...
            if code.starts_with("MDVR") {
                assert_eq!(
                    fanza_catalog::exact_library_transport_id(category.value(), code),
                    None
                );
            }
            let url =
                format!("https://pics.dmm.co.jp/digital/video/{content_id}/{content_id}pl.jpg");
            let document = format!(
                r#"{{"content_id":"{content_id}","dvd_id":{dvd_value},"title_ja":"Exact","images":{{"jacket_image":{{"large2":"{url}"}}}}}}"#
            );
            let proof = exact_legacy_cover(&document, &authority)
                .expect("the exact legacy tuple must be accepted");
            assert_eq!(proof.content_id, content_id);
            assert_eq!(proof.display_code, code);
            assert_eq!(proof.cover_url.as_deref(), Some(url.as_str()));

            let source = CoverSource {
                provider: "r18.dev",
                provider_id: proof.content_id,
                display_code: proof.display_code,
                url: url.clone(),
                aspect_ratio: 0.75,
            };
            assert!(cover_source_valid(&source, category, code));

            let fixture = CacheFixture::new(&format!("legacy-family-{content_id}"));
            let state = LibraryPresentationState::default();
            let first = resolve_cover_at_with(
                ...
                || {
                    resolve_cover_with(
                        &authority,
                        |_| Err(ProviderRequestError::SourceUnavailable),
                        |_| panic!("an unavailable JavDB source has no image"),
                        |_| Err(ProviderRequestError::SourceUnavailable),
                        |_| panic!("an unavailable FANZA source has no image"),
                        |_| Ok(document.clone()),
                        |_| Ok(jpeg(600, 800)),
                    )
                },
            )
            .expect("the exact legacy source must resolve for the current family");
            assert_eq!(&first[2..6], ["ready", "r18.dev", content_id, code]);
            fetch_cover_with(&state, &fixture.path, &authority, &first[6], |_| {
                panic!("the exact legacy bytes must already be retained")
            })
            .expect("the exact legacy cover must become a local cache hit");
            let restarted_state = LibraryPresentationState::default();
            let restarted = resolve_cover_at_with(
                &restarted_state,
                &fixture.path,
                ...
                101,
                || true,
                || panic!("restart must not repeat exact legacy provider work"),
            )
            .expect("the exact legacy proof and bytes must survive restart");
            fetch_cover_with(
                &restarted_state,
                &fixture.path,
                &authority,
                &restarted[6],
                |_| panic!("restart must not repeat the remote DMM image request"),
            )
            .expect("restart must return the application-owned legacy bytes");

            let crossed = document.replace(
                &format!("/{content_id}/{content_id}pl.jpg"),
                "/crossed00001/crossed00001pl.jpg",
            );
            assert_eq!(
                exact_legacy_cover(&crossed, &authority),
                Err(ProviderRequestError::Provider)
            );
            let wrong_display = document.replace(dvd_value, r#""WRONG-1""#);
            assert_eq!(
                exact_legacy_cover(&wrong_display, &authority),
                Err(ProviderRequestError::Provider)
            );
        }
    }
    ...
    #[test]
    fn later_legacy_cover_with_a_different_exact_display_is_a_conflict() {
        ...
        .expect("legacy display disagreement must remain a current response");
        assert_eq!(response[2], "unavailable");
        assert_eq!(&response[8..], ["", "", ""]);
    }
    ...
    fn fresh_cover_and_confirmed_miss_cache_survive_restart_until_exact_expiry() {
        ...
        fetch_cover_with(&state, &fixture.path, &authority, &ready[6], |_| {
            panic!("first resolved bytes must already be retained")
        })
        .expect("the first displayed cover must become a local cache hit");

        let second_state = LibraryPresentationState::default();
        let second_visit = resolve_cover_at_with(
            &second_state,
            &fixture.path,
            &authority,
            101,
            || true,
            || panic!("the second visit must not repeat provider discovery"),
        )
        .expect("the second visit must use the local hot cache");
        fetch_cover_with(
            &second_state,
            &fixture.path,
            &authority,
            &second_visit[6],
            |_| panic!("the second visit must not repeat remote image work"),
        )
        .expect("the second visit must return the application-owned bytes");
        assert_eq!(calls.get(), 1);
        ...
        let fetched = fetch_cover_with(&restarted, &fixture.path, &authority, &cached[6], |_| {
            panic!("restart must use the retained local bytes")
        })
        .expect("restart must reuse the retained local bytes");
    ...
    #[test]
    fn invalid_local_cover_bytes_are_removed_and_current_provider_resolution_replaces_them() {
        for failure in ["corrupt", "oversized", "crossed", "expired"] {
            let fixture = CacheFixture::new(&format!("local-cover-{failure}"));
            let authority = authority(LibraryPresentationCategory::Vr);
            let source = cover("cacheditem", 0.75);
            write_cache(
                &fixture.path,
                &[CacheEntry {
                    identity: authority.identity.clone(),
                    ...
                    code: authority.code.clone(),
                    identity_saved_at: 100,
                    verified_identity: Some(verified_javdb("cacheditem", &authority.code)),
                    cover_saved_at: 100,
                    cover_state: "ready",
                    cover: Some(source.clone()),
                    metadata_saved_at: 0,
                    metadata_state: "missing",
                    metadata: PresentationMetadata::default(),
                }],
            )
            .expect("the local cover proof must be written");
            write_cover_bytes_for_cached_entry(&fixture.path, &authority, 100);
            let cached_path = cover_bytes_path(&fixture.path, &authority, &source)
                .expect("the exact local byte path must be available");
            let now = if failure == "expired" {
                101 + COVER_TTL_SECONDS
            } else {
                101
            };
            match failure {
                "corrupt" => fs::write(&cached_path, vec![0_u8; COVER_MIN_BYTES])
                    .expect("the exact cached bytes must be corruptible"),
                "oversized" => fs::write(&cached_path, vec![0_u8; COVER_MAX_BYTES + 1])
                    .expect("the exact cached bytes must be replaceable"),
                "crossed" => {
                    let crossed = CoverSource {
                        provider_id: "anotheritem".to_owned(),
                        url: "https://tp.cmastd.com/anotheritem.jpg".to_owned(),
                        ..source.clone()
                    };
                    let mut entries = read_cache(&fixture.path).expect("the proof cache is valid");
                    entries[0].cover = Some(crossed);
                    write_cache(&fixture.path, &entries)
                        .expect("the crossed proof fixture must be written");
                }
                "expired" => {}
                _ => unreachable!(),
            }
            ...
            let replacement = resolve_cover_at_with(
                ...
                || {
                    ...
                    (
                        Some((
                            CoverSource {
                                provider_id: "cacheditem".to_owned(),
                                url: "https://tp.cmastd.com/replacementitem.jpg".to_owned(),
                                ..source.clone()
                            },
                            jpeg(600, 800),
                        )),
                        Some(identity_evidence(verified_javdb(
                            "cacheditem",
                            &authority.code,
                        ))),
                        false,
            ...
            .expect("invalid local bytes must permit fresh current resolution");
            assert_eq!(provider_calls.get(), 1);
            assert!(!cached_path.exists());
            fetch_cover_with(&state, &fixture.path, &authority, &replacement[6], |_| {
                panic!("the replacement bytes are already retained")
            })
            .expect("the replacement cover must become the new local hit");
        }
    }

    #[cfg(unix)]
    #[test]
    fn symlinked_local_cover_bytes_are_rejected_and_removed() {
        use std::os::unix::fs::symlink;

        let fixture = CacheFixture::new("local-cover-symlink");
        let authority = authority(LibraryPresentationCategory::Vr);
        let source = cover("cacheditem", 0.75);
        write_cache(
            &fixture.path,
        ...
        let cached_path = cover_bytes_path(&fixture.path, &authority, &source)
            .expect("the exact local byte path must be available");
        let target = fixture.directory.join("unrelated-cover");
        fs::write(&target, jpeg(600, 800)).expect("the unrelated bytes must be written");
        fs::remove_file(&cached_path).expect("the cached bytes must be removable");
        symlink(&target, &cached_path).expect("the cached path must be replaceable by a symlink");
        ...
        .expect("a symlinked local cover must be treated as a cache miss");
        assert_eq!(provider_calls.get(), 1);
        assert!(!cached_path.exists());
    ...
    fn cover_authority_is_released_on_cancellation_and_consumed_on_fetch_failure() {
        ...
        assert_eq!(
            fetch_cover_with(&state, &fixture.path, &authority, authority_id, |_| {
        ...
        .expect("an unconsumed cancelled source must be resolved again");
        restarted
            .0
            .lock()
            .unwrap()
            .covers
            .get_mut(&cached[6])
            .expect("the current authority must exist")
            .bytes = None;
        assert_eq!(
            fetch_cover_with(&restarted, &fixture.path, &authority, &cached[6], |_| {
                Err(ProviderRequestError::Network)
            }),
        ...
        resolve_cover_at_with_request(
            ...
            |_| {
                (
                    None,
                    Some(PresentationMetadata {
                        verified_identity: Some(verified_javdb("currentitem", &authority.code)),
                        source: Some("JavDB".to_owned()),
                        provider_id: Some("currentitem".to_owned()),
                        display_code: Some(authority.code.clone()),
    ...
    fn metadata_seed_cannot_cross_replacement_or_obsolete_cancellation() {
        ...
        assert_eq!(metadata[5], "currentitem");
    ...
    fn fourteen_current_cover_authorities_remain_fetchable_until_consumed() {
        let fixture = CacheFixture::new("fourteen-cover-authorities");
        let state = LibraryPresentationState::default();
        for first in (1..=14).step_by(MAX_RETAINED_COVER_BYTES) {
            ...
            for (authority, cover_authority_id) in current {
                let fetched = fetch_cover_with(
                    &state,
                    &fixture.path,
                    &authority,
                    &cover_authority_id,
                    |_| panic!("current cover bytes must be consumed before another batch"),
                )
    ...
    fn invalidated_cached_source_is_not_retried_and_provider_resolution_can_replace_it() {
        ...
        let failed_source = read_cache(&fixture.path)
            .expect("the cached proof must remain readable")[0]
            .cover
            .clone()
            .expect("the current cover proof must be retained");
        let failed_bytes = cover_bytes_path(&fixture.path, &authority, &failed_source)
            .expect("the exact cached byte path must be available");
        assert!(failed_bytes.exists());
    ...
    fn cover_and_metadata_identity_conflicts_clear_durable_presentation() {
        for (index, current_identity, metadata_identity) in [
            ...
            let state = LibraryPresentationState::default();
            begin_cover_request(&state, authority.category, &authority.code, 1)
                .expect("the metadata conflict request must begin");
            let response = resolve_metadata_at_with(
                &state,
                &fixture.path,
                &authority,
    ...
    #[test]
    fn legacy_cache_proofs_reject_crossed_content_and_reuse_exact_content() {
        let authority = LibraryItemAuthority {
            category: LibraryPresentationCategory::Adult,
            identity: "9".repeat(40),
            code: "CAWB-1".to_owned(),
        ...
        for (label, cover, metadata) in [
            (
                ...
                Some(CoverSource {
                    provider: "r18.dev",
                    provider_id: "cawb00002".to_owned(),
                    display_code: "CAWB-1".to_owned(),
            ...
            (
                "wrong-metadata-content",
                None,
                PresentationMetadata {
                    source: Some("r18.dev".to_owned()),
                    provider_id: Some("cawb00002".to_owned()),
                    legacy_provider_id: Some("cawb00002".to_owned()),
                    legacy_display_code: Some("CAWB-2".to_owned()),
                    display_code: Some("CAWB-1".to_owned()),
                    title: Some("Crossed title".to_owned()),
                    ..PresentationMetadata::default()
                },
            ),
        ] {
            let fixture = CacheFixture::new(label);
            let metadata_ready = metadata != PresentationMetadata::default();
            write_cache(
                &fixture.path,
                &[CacheEntry {
                    identity: authority.identity.clone(),
                    ...
                    identity_saved_at: 0,
                    verified_identity: None,
                    cover_saved_at: cover.as_ref().map_or(0, |_| 100),
                    cover_state: if cover.is_some() { "ready" } else { "missing" },
                    cover,
                    metadata_saved_at: if metadata_ready { 100 } else { 0 },
                    metadata_state: if metadata_ready { "ready" } else { "missing" },
                    metadata,
                }],
            )
            .expect("a structurally encoded crossed legacy fixture must be written");
            assert!(read_cache(&fixture.path).is_err());
            assert_invalid_cache_refreshes(&fixture.path, &authority);
        }

        let fixture = CacheFixture::new("exact-legacy-proof");
        let metadata = PresentationMetadata {
            source: Some("r18.dev".to_owned()),
            provider_id: Some("cawb00001".to_owned()),
            legacy_provider_id: Some("cawb00001".to_owned()),
            verified_identity: Some(VerifiedDisplayIdentity {
                provider: "r18.dev",
                provider_id: "cawb00001".to_owned(),
                display_code: "CAWB-001".to_owned(),
            }),
            legacy_display_code: Some("CAWB-001".to_owned()),
            display_code: Some("CAWB-001".to_owned()),
            title: Some("Exact legacy title".to_owned()),
            ..PresentationMetadata::default()
        };
        write_cache(
            &fixture.path,
            &[CacheEntry {
                ...
                metadata_saved_at: 100,
                metadata_state: "ready",
                metadata,
            }],
        )
        .expect("the exact legacy proof cache must be written");
        write_cover_bytes_for_cached_entry(&fixture.path, &authority, 100);
        let cover = resolve_cover_at_with(
            &LibraryPresentationState::default(),
            &fixture.path,
            &authority,
            101,
            || true,
            || panic!("an exact current legacy cover cache must skip provider resolution"),
        )
        .expect("the exact legacy cover proof must be reusable");
        assert_eq!(&cover[2..6], ["ready", "r18.dev", "cawb00001", "CAWB-001"]);
        ...
        let metadata = resolve_metadata_at_with(
            ...
            101,
            || true,
            || panic!("an exact current legacy metadata cache must skip provider resolution"),
        )
        .expect("the exact legacy metadata proof must be reusable");
        assert_eq!(metadata[2], "automatic");
        assert_eq!(metadata[7], "r18.dev");
        assert_eq!(metadata[8], "cawb00001");
        assert_eq!(metadata[metadata.len() - 2], "cawb00001");
        assert_eq!(metadata.last().map(String::as_str), Some("CAWB-001"));
    }
    ...
    #[test]
    fn standalone_legacy_display_identity_survives_no_cover_cache_and_restart() {
        let fixture = CacheFixture::new("legacy-display-no-cover");
        let authority = LibraryItemAuthority {
            category: LibraryPresentationCategory::Adult,
            identity: "a".repeat(40),
            code: "CAWB-1".to_owned(),
        ...
        let resolve = || {
            resolve_cover_with(
                &authority,
                |_| Ok(r#"{"success":1,"data":{"movies":[]}}"#.to_owned()),
                |_| panic!("a JavDB no-match has no image"),
                |_| Ok(r#"{"data":{"c0":null}}"#.to_owned()),
                |_| panic!("a FANZA no-match has no image"),
                |_| {
                    Ok(
                        r#"{"content_id":"cawb00001","dvd_id":"CAWB-001","images":null}"#
                            .to_owned(),
                    )
                },
                |_| panic!("a legacy no-cover proof has no image"),
            )
        };
        let first = resolve_cover_at_with(
            &LibraryPresentationState::default(),
        ...
        .expect("the explicit padded legacy display must establish identity");
        assert_eq!(first[2], "missing");
        assert_eq!(&first[8..], ["r18.dev", "cawb00001", "CAWB-001"]);
        ...
        let restarted = resolve_cover_at_with(
            ...
            101,
            || true,
            || panic!("the exact no-cover identity must survive restart"),
        )
        .expect("the exact no-cover identity cache must be reusable");
        assert_eq!(restarted, first);

        let conflict_fixture = CacheFixture::new("legacy-display-conflict");
        let conflicting = resolve_cover_at_with(
            &LibraryPresentationState::default(),
            ...
            || {
                resolve_cover_with(
                    ...
                    |url| {
                        Ok((if url.contains("search") {
                            javdb_listing("CAWB-0001")
                        } else {
                            javdb_detail("CAWB-0001", authority.category)
                        })
                        .replace(
                            r#""cover_url":"https://tp.cmastd.com/exact.jpg""#,
                            r#""cover_url":null"#,
                    ...
                    |_| panic!("the JavDB fixture has no cover"),
                    |_| Ok(r#"{"data":{"c0":null}}"#.to_owned()),
                    |_| panic!("the FANZA fixture has no cover"),
                    |_| {
                        Ok(
                            r#"{"content_id":"cawb00001","dvd_id":"CAWB-001","images":null}"#
                                .to_owned(),
                    ...
                    |_| panic!("the conflicting legacy proof has no cover"),
                )
            },
        )
        .expect("exact-display disagreement must become a current conflict");
        assert_eq!(conflicting[2], "unavailable");
        assert_eq!(&conflicting[8..], ["", "", ""]);
        assert_eq!(
            read_cache(&conflict_fixture.path).expect("the conflict cache must be valid")[0]
                .cover_state,
            "conflict"
        );
        let restarted_conflict = resolve_metadata_at_with(
            &LibraryPresentationState::default(),
            &conflict_fixture.path,
            &authority,
            None,
            101,
            || true,
            || panic!("restart must not discard the durable display conflict"),
        )
        .expect("restart must retain the unavailable conflict");
        assert_eq!(restarted_conflict[2], "unavailable");
    }

    #[test]
    fn persistent_proofs_evict_only_obsolete_records_at_the_count_bound() {
        let fixture = CacheFixture::new("proof-count-bound");
        let mut entries = (1..=CACHE_MAX_ENTRIES)
            .map(|number| CacheEntry {
                identity: format!("{number:040x}"),
                category: LibraryPresentationCategory::Adult,
                code: format!("ADLT-{number}"),
                identity_saved_at: 0,
                verified_identity: None,
                cover_saved_at: number as u64,
                cover_state: "missing",
                cover: None,
                metadata_saved_at: 0,
                metadata_state: "missing",
            ...
            .collect::<Vec<_>>();
        write_cache(&fixture.path, &entries).expect("the full proof cache must be written");
        let protected = HashSet::from([(LibraryPresentationCategory::Adult, "ADLT-1".to_owned())]);
        let newest = CacheEntry {
            identity: "f".repeat(40),
            category: LibraryPresentationCategory::Adult,
            code: "ADLT-999".to_owned(),
            identity_saved_at: 0,
            verified_identity: None,
            cover_saved_at: 999,
            cover_state: "missing",
        ...
        merge_cache_entry_with_protected(&fixture.path, newest, &protected)
            .expect("an obsolete proof must be evicted for current work");
        entries = read_cache(&fixture.path).expect("the bounded proof cache must remain valid");
        assert_eq!(entries.len(), CACHE_MAX_ENTRIES);
        assert!(entries.iter().any(|entry| entry.code == "ADLT-1"));
        assert!(entries.iter().any(|entry| entry.code == "ADLT-999"));
        assert!(!entries.iter().any(|entry| entry.code == "ADLT-2"));
    }

    #[test]
    fn local_cover_bytes_evict_obsolete_records_for_count_and_total_byte_limits() {
        for (label, maximum_files, maximum_bytes) in [
            ("file-count", 1, u64::MAX),
            ("total-bytes", 8, jpeg(600, 800).len() as u64 + 1),
        ] {
            let fixture = CacheFixture::new(&format!("cover-byte-{label}"));
            let first = authority(LibraryPresentationCategory::Vr);
            let second = LibraryItemAuthority {
                identity: "b".repeat(40),
                code: "MDVR-420".to_owned(),
                product_identity: "MDVR-420".to_owned(),
                ..first.clone()
            };
            let first_source = cover("firstitem", 0.75);
            let second_source = CoverSource {
                provider_id: "seconditem".to_owned(),
                display_code: second.code.clone(),
                url: "https://tp.cmastd.com/seconditem.jpg".to_owned(),
                ..first_source.clone()
            };
            let entries = [
                CacheEntry {
                    ...
                    identity_saved_at: 10,
                    verified_identity: Some(verified_javdb("firstitem", &first.code)),
                    cover_saved_at: 10,
                    cover_state: "ready",
                    cover: Some(first_source.clone()),
                    metadata_saved_at: 0,
                    metadata_state: "missing",
                    metadata: PresentationMetadata::default(),
                },
                CacheEntry {
                    ...
                    identity_saved_at: 20,
                    verified_identity: Some(verified_javdb("seconditem", &second.code)),
                    cover_saved_at: 20,
                    cover_state: "ready",
                    cover: Some(second_source.clone()),
                    metadata_saved_at: 0,
                    metadata_state: "missing",
                    metadata: PresentationMetadata::default(),
                },
            ];
            write_cache(&fixture.path, &entries).expect("both exact proofs must be durable");
            write_cached_cover_bytes_with_limits(
                &fixture.path,
                &first,
                &first_source,
            ...
            .expect("the first local cover must fit");
            let current_entries = read_cache(&fixture.path).expect("proofs must remain readable");
            write_cached_cover_bytes_with_limits(
                &fixture.path,
                &second,
                &second_source,
                ...
                &current_entries,
                21,
                &HashSet::from([(LibraryPresentationCategory::Vr, second.code.clone())]),
                maximum_files,
                maximum_bytes,
            )
            .expect("obsolete local bytes must make room for the displayed current cover");
            assert!(read_cached_cover_bytes(&fixture.path, &first, &first_source).is_err());
            assert!(read_cached_cover_bytes(&fixture.path, &second, &second_source).is_ok());
        }
    }

    #[test]
    fn failed_local_cover_write_fails_the_byte_fetch_instead_of_succeeding_silently() {
        let fixture = CacheFixture::new("forced-cover-write-failure");
        let authority = authority(LibraryPresentationCategory::Vr);
        let state = LibraryPresentationState::default();
        let response = resolve_cover_at_with(
            &state,
            ...
            || {
                (
                    Some((cover("item", 0.75), jpeg(600, 800))),
                    Some(identity_evidence(verified_javdb("item", &authority.code))),
                    false,
                )
            },
        )
        .expect("the current cover proof must resolve");
        let entries = read_cache(&fixture.path).expect("the cover proof must be durable");
        let source = entries[0]
            .cover
            .as_ref()
            .expect("the cover source must exist");
        let target = cover_bytes_path(&fixture.path, &authority, source)
            .expect("the local byte target must be deterministic");
        fs::create_dir_all(&target).expect("a deterministic write blocker must be created");
        assert_eq!(
            fetch_cover_with(&state, &fixture.path, &authority, &response[6], |_| {
                panic!("resolved bytes must not be fetched remotely")
            }),
            Err(LIBRARY_PRESENTATION_FAILED)
        );
    }
    ...
    #[test]
    fn metadata_unavailability_is_not_a_local_only_miss_and_retry_can_succeed() {
        let fixture = CacheFixture::new("metadata-unavailable-retry");
        let authority = authority(LibraryPresentationCategory::Adult);
        let unavailable = resolve_metadata_at_with(
            &LibraryPresentationState::default(),
            &fixture.path,
            &authority,
            None,
        ...
        .expect("metadata source unavailability must remain a response state");
        assert_eq!(unavailable[2], "unavailable");
        assert!(!fixture.path.exists());

        let mixed = resolve_metadata_with(
            &authority,
            |_| Ok(r#"{"success":1,"data":{"movies":[]}}"#.to_owned()),
            |_| Err(ProviderRequestError::Network),
        );
        ...
        let metadata = PresentationMetadata {
            verified_identity: Some(verified_javdb("retryitem", &authority.code)),
            source: Some("JavDB".to_owned()),
            provider_id: Some("retryitem".to_owned()),
            display_code: Some(authority.code.clone()),
            title: Some("Recovered metadata".to_owned()),
            ..PresentationMetadata::default()
        };
        let recovered = resolve_metadata_at_with(
            &LibraryPresentationState::default(),
            &fixture.path,
            &authority,
            None,
        ...
        .expect("a later exact metadata retry must recover");
        assert_eq!(recovered[2], "automatic");
        assert_eq!(recovered[10], "Recovered metadata");
    }
}
