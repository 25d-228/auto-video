...
const CACHE_VERSION: &str = "library-presentation-v2";
...
fn resolve_cover_with(
    ...
    let mut verified_display_code = None;
    match fetch_exact_javdb_item(authority, &mut javdb_document) {
        Ok(Some(item)) => {
            verified_display_code = Some(item.display_code.clone());
    ...
    match fanza_catalog::fetch_exact_library_item_with(
        ...
        Ok(Some(item)) => {
            let display_code = verified_display_code
                .clone()
                .unwrap_or_else(|| item.display_code.clone());
            if let Some(url) = item.cover_url {
                        ...
                        return (
                            Some((
                                CoverSource {
                                    ...
                                    display_code: verified_display_code
                                        .clone()
                                        .unwrap_or_else(|| authority.code.clone()),
                                    url,
                                    aspect_ratio: ratio,
                                },
                                bytes,
...
fn cache_entry_valid(entry: &CacheEntry) -> bool {
    entry.identity.len() == 40
        ...
        && entry.cover.as_ref().is_none_or(|source| {
            crate::vr_torrent::canonical_product_code(&source.display_code)
                == crate::vr_torrent::canonical_product_code(&entry.code)
        })
        && entry.metadata.display_code.as_ref().is_none_or(|display| {
            crate::vr_torrent::canonical_product_code(display)
                == crate::vr_torrent::canonical_product_code(&entry.code)
        })
    ...
    for line in lines {
        ...
        let cover = if fields[4] == "ready" {
            Some(CoverSource {
                ...
                display_code: decode_text(fields[7]).ok_or(())?,
                url: decode_text(fields[8]).ok_or(())?,
                aspect_ratio: fields[9].parse().map_err(|_| ())?,
            })
        } else {
            None
        };
        let metadata = PresentationMetadata {
            source: optional_text(fields[12]),
            provider_id: optional_text(fields[13]),
            display_code: optional_text(fields[14]),
            title: optional_text(fields[15]),
            date: optional_text(fields[16]),
            runtime: optional_text(fields[17]),
            cast: fields[18]
                .split(',')
        ...
        let entry = CacheEntry {
            ...
            cover,
            metadata_saved_at: fields[10].parse().map_err(|_| ())?,
            metadata_state: match fields[11] {
                "ready" => "ready",
                "missing" => "missing",
                _ => return Err(()),
            },
                ...
                (
                    Some((
                        CoverSource {
                            ...
                            display_code: "3DSVR-01871".to_owned(),
