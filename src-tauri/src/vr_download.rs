...
const CLEANUP_RECOVERY_HEADER: &[u8] = b"AUTO_VIDEO_TRANSFER_CLEANUP_V2\n";
const LEGACY_CLEANUP_RECOVERY_HEADER: &[u8] = b"AUTO_VIDEO_TRANSFER_CLEANUP_V1\n";
...
const CLEANUP_RECOVERY_PREFIX: &str = ".auto-video-transfer-cleanup-";
const PERSISTENCE_REPLACEMENT_SUFFIX: &str = ".next";
const TERMINAL_RECOVERY_DIRECTORY: &str = ".auto-video-transfer-terminal-recovery";
const CLEANUP_RECOVERY_DIRECTORY: &str = ".auto-video-transfer-cleanup-recovery";
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum CleanupMutationPhase {
    TemporaryCreationPrepared,
    TemporaryCreated,
    SwapPrepared,
    Swapped,
    ExactDeletionPrepared,
    ExactDeleted,
    TemporaryRelocationPrepared,
    TemporaryRelocated,
    TemporaryCleanupPrepared,
}

impl CleanupMutationPhase {
    fn as_str(self) -> &'static str {
        match self {
            Self::TemporaryCreationPrepared => "temporary-creation-prepared",
            Self::TemporaryCreated => "temporary-created",
            Self::SwapPrepared => "swap-prepared",
            Self::Swapped => "swapped",
            Self::ExactDeletionPrepared => "exact-deletion-prepared",
            Self::ExactDeleted => "exact-deleted",
            Self::TemporaryRelocationPrepared => "temporary-relocation-prepared",
            Self::TemporaryRelocated => "temporary-relocated",
            Self::TemporaryCleanupPrepared => "temporary-cleanup-prepared",
        }
    }

    fn from_str(value: &str) -> Option<Self> {
        match value {
            "temporary-creation-prepared" => Some(Self::TemporaryCreationPrepared),
            "temporary-created" => Some(Self::TemporaryCreated),
            "swap-prepared" => Some(Self::SwapPrepared),
            "swapped" => Some(Self::Swapped),
            "exact-deletion-prepared" => Some(Self::ExactDeletionPrepared),
            "exact-deleted" => Some(Self::ExactDeleted),
            "temporary-relocation-prepared" => Some(Self::TemporaryRelocationPrepared),
            "temporary-relocated" => Some(Self::TemporaryRelocated),
            "temporary-cleanup-prepared" => Some(Self::TemporaryCleanupPrepared),
            _ => None,
        }
    }
}
...
struct CleanupMutation {
    temporary_path: PathBuf,
    temporary_fingerprint: Option<String>,
    phase: CleanupMutationPhase,
}
...
struct CleanupRecovery {
    ...
    mutations: Vec<Option<CleanupMutation>>,
    stopped: bool,
}
...
fn cleanup_recovery_path(
    ...
    Ok(cleanup_recovery_directory(persistence_path)?.join(format!(
        "{}{}{}",
        CLEANUP_RECOVERY_PREFIX, record.transfer_id, TERMINAL_RECOVERY_SUFFIX
    )))
}
...
fn cleanup_temporary_path(
    ...
    Ok(current_target(record, selected_index)?
        .parent()
        .ok_or(VR_DOWNLOAD_PERSISTENCE_FAILED)?
        .join(format!(
            ".auto-video-cleanup-{}-{selected_index}.delete",
            record.transfer_id
        )))
}

fn encode_cleanup_mutations(recovery: &CleanupRecovery) -> String {
    recovery
        .mutations
        .iter()
        .map(|mutation| match mutation {
            ...
            Some(mutation) => format!(
                ...
                encode_hex(mutation.temporary_path.to_string_lossy().as_bytes()),
                mutation
...
fn decode_cleanup_mutations(value: &[u8]) -> Option<Vec<Option<CleanupMutation>>> {
    std::str::from_utf8(value)
        ...
        .map(|entry| {
            ...
            let phase = CleanupMutationPhase::from_str(fields.next()?)?;
            let temporary_path = PathBuf::from(decode_text(fields.next()?.as_bytes())?);
            let temporary_fingerprint = match fields.next()? {
                "-" => None,
                fingerprint => Some(decode_text(fingerprint.as_bytes())?),
            };
            if fields.next().is_some() {
                return None;
            }
            Some(Some(CleanupMutation {
                temporary_path,
                temporary_fingerprint,
                phase,
            }))
...
fn validate_cleanup_mutations(recovery: &CleanupRecovery) -> Result<(), &'static str> {
    if recovery.mutations.len() != recovery.record.selected_files.len()
        || recovery
            .mutations
            .iter()
            .filter(|mutation| mutation.is_some())
            .count()
            > 1
    {
        return Err(VR_DOWNLOAD_PERSISTENCE_FAILED);
    }
    for (selected_index, mutation) in recovery.mutations.iter().enumerate() {
        let Some(mutation) = mutation else { continue };
        if recovery.files[selected_index] != CleanupFileState::Present
            || mutation.temporary_path != cleanup_temporary_path(&recovery.record, selected_index)?
            || (mutation.phase == CleanupMutationPhase::TemporaryCreationPrepared
                && mutation.temporary_fingerprint.is_some())
            || (mutation.phase != CleanupMutationPhase::TemporaryCreationPrepared
                && mutation.temporary_fingerprint.is_none())
        {
            return Err(VR_DOWNLOAD_PERSISTENCE_FAILED);
        }
    }
...
fn encoded_cleanup_recovery(recovery: &CleanupRecovery) -> Result<Vec<u8>, &'static str> {
    ...
    validate_cleanup_mutations(recovery)?;
    let stopped = if recovery.stopped {
        b"true".as_slice()
    } else {
        b"false".as_slice()
    };
    let states = recovery
        ...
        .collect::<Vec<_>>()
        .join(",");
    let mutations = encode_cleanup_mutations(recovery);
    let encoded_record = encode_transfer(&recovery.record)?;
    let mut checksum_input = stopped.to_vec();
    checksum_input.push(b'\n');
    checksum_input.extend_from_slice(states.as_bytes());
    checksum_input.push(b'\n');
    checksum_input.extend_from_slice(mutations.as_bytes());
    checksum_input.push(b'\n');
    checksum_input.extend_from_slice(&encoded_record);
...
fn parse_cleanup_recovery(persistence_path: &Path, path: &Path) -> Option<CleanupRecovery> {
    ...
    let bytes = fs::read(path).ok()?;
    let (legacy, fields) = if let Some(bytes) = bytes.strip_prefix(CLEANUP_RECOVERY_HEADER) {
        (
            false,
        ...
        (
            true,
            bytes
                .strip_prefix(LEGACY_CLEANUP_RECOVERY_HEADER)?
                .split(|byte| *byte == b'\n')
                .collect::<Vec<_>>(),
        )
    };
    let (checksum, stopped, states, mutations, encoded_record, trailing) = if legacy {
        let [checksum, stopped, states, encoded_record, trailing] = fields.as_slice() else {
            return None;
        };
        (
            *checksum,
        ...
        let [checksum, stopped, states, mutations, encoded_record, trailing] = fields.as_slice()
        else {
            return None;
        };
        (
    ...
    let mut checksum_input = stopped.to_vec();
    checksum_input.push(b'\n');
    checksum_input.extend_from_slice(states);
    checksum_input.push(b'\n');
    if let Some(mutations) = mutations {
        checksum_input.extend_from_slice(mutations);
        checksum_input.push(b'\n');
    }
    checksum_input.extend_from_slice(encoded_record);
    if checksum != hex_sha1(&checksum_input).as_bytes() {
    ...
    let mutations = match mutations {
        Some(mutations) => decode_cleanup_mutations(mutations)?,
    ...
    if record.state != TransferState::Cleanup
        ...
        || mutations.len() != record.selected_files.len()
    ...
    validate_cleanup_mutations(&recovery).ok()?;
    Some(recovery)
}
...
fn cleanup_recovery_paths(persistence_path: &Path) -> Result<Vec<PathBuf>, &'static str> {
    ...
    for entry in entries {
        ...
        let Some(transfer_id) = name
            .strip_prefix(CLEANUP_RECOVERY_PREFIX)
            .and_then(|name| name.strip_suffix(TERMINAL_RECOVERY_SUFFIX))
        else {
            continue;
        };
...
fn write_cleanup_recovery(
    ...
    match fs::symlink_metadata(&path) {
        Ok(metadata) => {
            ...
            if metadata.file_type().is_symlink()
                || !metadata.is_file()
                || !same_transfer_authority(&existing.record, &recovery.record)
            {
                return Err(VR_DOWNLOAD_PERSISTENCE_FAILED);
            }
        }
...
fn remove_cleanup_recovery(
    ...
    if !same_transfer_authority(&current.record, &expected.record)
        ...
        || current.mutations != expected.mutations
        || current.stopped != expected.stopped
    {
        return Err(VR_DOWNLOAD_PERSISTENCE_FAILED);
    }
    fs::remove_file(&path).map_err(|_| VR_DOWNLOAD_PERSISTENCE_FAILED)?;
        ...
        .map_err(|_| VrLibraryTrashOwnershipError::Unavailable)?
    {
        ...
        if transfer_record_owns_path(&recovery.record, requested_path)?
            || recovery
                ...
                .iter()
                .flatten()
                .any(|mutation| mutation.temporary_path == requested_path)
        {
            return Ok(true);
        }
    }
...
#[cfg(target_os = "macos")]
fn macos_rename_exact(source: &Path, destination: &Path, flags: u32) -> io::Result<()> {
    use std::{ffi::CString, os::unix::ffi::OsStrExt};

    const AT_FDCWD: i32 = -2;
    let source = CString::new(source.as_os_str().as_bytes())
        .map_err(|_| io::Error::other("cleanup source path contains a null byte"))?;
    let destination = CString::new(destination.as_os_str().as_bytes())
        .map_err(|_| io::Error::other("cleanup destination path contains a null byte"))?;
    let result = unsafe {
        renameatx_np(
            AT_FDCWD,
            source.as_ptr(),
...
#[cfg(target_os = "macos")]
fn open_macos_cleanup_file(path: &Path) -> io::Result<File> {
...
#[cfg(target_os = "macos")]
fn macos_cleanup_path_fingerprint(path: &Path) -> Result<Option<String>, &'static str> {
    match open_macos_cleanup_file(path) {
        Ok(file) => open_file_fingerprint(&file)
            .map(Some)
...
#[cfg(target_os = "macos")]
fn remove_exact_macos_cleanup_path(
    path: &Path,
    expected_fingerprint: &str,
) -> Result<(), &'static str> {
    match macos_cleanup_path_fingerprint(path)? {
        None => Ok(()),
        ...
        Some(_) => Err(VR_DOWNLOAD_STALE),
    }
}

#[cfg(target_os = "macos")]
fn persist_cleanup_mutation_phase(
    persistence_path: &Path,
    recovery: &mut CleanupRecovery,
    selected_index: usize,
    temporary_path: &Path,
    temporary_fingerprint: Option<&str>,
    phase: CleanupMutationPhase,
) -> Result<(), &'static str> {
    recovery.mutations[selected_index] = Some(CleanupMutation {
        temporary_path: temporary_path.to_owned(),
        temporary_fingerprint: temporary_fingerprint.map(str::to_owned),
        phase,
    });
...
#[cfg(target_os = "macos")]
fn clear_cleanup_mutation(
    persistence_path: &Path,
    recovery: &mut CleanupRecovery,
    selected_index: usize,
    state: CleanupFileState,
) -> Result<(), &'static str> {
    recovery.files[selected_index] = state;
    recovery.mutations[selected_index] = None;
    write_cleanup_recovery(persistence_path, recovery)
}

#[cfg(target_os = "macos")]
fn delete_exact_cleanup_file_with(
    persistence_path: &Path,
    recovery: &mut CleanupRecovery,
    selected_index: usize,
    before_swap: impl FnOnce() -> io::Result<()>,
    mut after_phase: impl FnMut(CleanupMutationPhase) -> io::Result<()>,
) -> Result<(), &'static str> {
    ...
    let target_file = open_macos_cleanup_file(&target).map_err(|_| VR_DOWNLOAD_STALE)?;
    if open_file_fingerprint(&target_file).map_err(|_| VR_DOWNLOAD_STALE)? != expected_fingerprint {
        return Err(VR_DOWNLOAD_STALE);
    }
    let temporary_path = cleanup_temporary_path(&recovery.record, selected_index)?;
    persist_cleanup_mutation_phase(
        persistence_path,
        recovery,
        selected_index,
        &temporary_path,
        None,
        CleanupMutationPhase::TemporaryCreationPrepared,
    )?;
    ...
    let placeholder_fingerprint =
        open_file_fingerprint(&placeholder_file).map_err(|_| VR_DOWNLOAD_CLEANUP_FAILED)?;
    persist_cleanup_mutation_phase(
        persistence_path,
        recovery,
        selected_index,
        &temporary_path,
        Some(&placeholder_fingerprint),
        CleanupMutationPhase::TemporaryCreated,
    )?;
    after_phase(CleanupMutationPhase::TemporaryCreated).map_err(|_| VR_DOWNLOAD_CLEANUP_FAILED)?;
    before_swap().map_err(|_| VR_DOWNLOAD_CLEANUP_FAILED)?;
    persist_cleanup_mutation_phase(
        persistence_path,
        recovery,
        selected_index,
        &temporary_path,
        Some(&placeholder_fingerprint),
        CleanupMutationPhase::SwapPrepared,
    )?;

    macos_rename_exact(&target, &temporary_path, RENAME_SWAP)
        .map_err(|_| VR_DOWNLOAD_CLEANUP_FAILED)?;
    let swapped_fingerprint = match open_macos_cleanup_file(&temporary_path)
        .and_then(|file| open_file_fingerprint(&file))
    {
    ...
    if swapped_fingerprint != expected_fingerprint {
        let _ = reconcile_macos_cleanup_mutation(persistence_path, recovery, selected_index);
        return Err(VR_DOWNLOAD_STALE);
    }
    persist_cleanup_mutation_phase(
        persistence_path,
        ...
        &temporary_path,
        Some(&placeholder_fingerprint),
        CleanupMutationPhase::Swapped,
    )?;
    after_phase(CleanupMutationPhase::Swapped).map_err(|_| VR_DOWNLOAD_CLEANUP_FAILED)?;
    ...
    persist_cleanup_mutation_phase(
        ...
        &temporary_path,
        Some(&placeholder_fingerprint),
        CleanupMutationPhase::ExactDeletionPrepared,
    )?;
    remove_exact_macos_cleanup_path(&temporary_path, &expected_fingerprint)?;
    drop(target_file);
    persist_cleanup_mutation_phase(
        persistence_path,
        recovery,
        ...
        &temporary_path,
        Some(&placeholder_fingerprint),
        CleanupMutationPhase::ExactDeleted,
    )?;
    after_phase(CleanupMutationPhase::ExactDeleted).map_err(|_| VR_DOWNLOAD_CLEANUP_FAILED)?;
    ...
    persist_cleanup_mutation_phase(
        ...
        &temporary_path,
        Some(&placeholder_fingerprint),
        CleanupMutationPhase::TemporaryRelocationPrepared,
    )?;
    macos_rename_exact(&target, &temporary_path, RENAME_EXCL)
        .map_err(|_| VR_DOWNLOAD_CLEANUP_FAILED)?;
    persist_cleanup_mutation_phase(
        persistence_path,
        recovery,
        ...
        &temporary_path,
        Some(&placeholder_fingerprint),
        CleanupMutationPhase::TemporaryRelocated,
    )?;
    after_phase(CleanupMutationPhase::TemporaryRelocated)
        .map_err(|_| VR_DOWNLOAD_CLEANUP_FAILED)?;
    persist_cleanup_mutation_phase(
        persistence_path,
        recovery,
        ...
        &temporary_path,
        Some(&placeholder_fingerprint),
        CleanupMutationPhase::TemporaryCleanupPrepared,
    )?;
    after_phase(CleanupMutationPhase::TemporaryCleanupPrepared)
        .map_err(|_| VR_DOWNLOAD_CLEANUP_FAILED)?;
    drop(placeholder_file);
    remove_exact_macos_cleanup_path(&temporary_path, &placeholder_fingerprint)?;
    sync_parent_directory(&target);
    Ok(())
}

#[cfg(target_os = "macos")]
fn finish_reconciled_macos_deletion(
    persistence_path: &Path,
    recovery: &mut CleanupRecovery,
    selected_index: usize,
) -> Result<(), &'static str> {
    ...
    let target = current_target(&recovery.record, selected_index)?;
    let expected_fingerprint = recovery.record.fingerprints[selected_index].clone();
    let mutation = recovery.mutations[selected_index]
        .clone()
        .ok_or(VR_DOWNLOAD_PERSISTENCE_FAILED)?;
    let temporary_path = mutation.temporary_path;
    let placeholder_fingerprint = mutation
        .temporary_fingerprint
        .ok_or(VR_DOWNLOAD_PERSISTENCE_FAILED)?;

    if macos_cleanup_path_fingerprint(&temporary_path)?.as_deref()
    ...
    let mut target_fingerprint = macos_cleanup_path_fingerprint(&target)?;
    let mut temporary_fingerprint = macos_cleanup_path_fingerprint(&temporary_path)?;
    if temporary_fingerprint
        .as_deref()
        .is_some_and(|fingerprint| fingerprint != placeholder_fingerprint)
    {
        return Err(VR_DOWNLOAD_STALE);
    }
    if target_fingerprint.as_deref() == Some(placeholder_fingerprint.as_str()) {
        if temporary_fingerprint.is_some() {
            return Err(VR_DOWNLOAD_STALE);
        }
        persist_cleanup_mutation_phase(
            persistence_path,
        ...
        target_fingerprint = macos_cleanup_path_fingerprint(&target)?;
        temporary_fingerprint = macos_cleanup_path_fingerprint(&temporary_path)?;
    }
    if target_fingerprint.as_deref() == Some(expected_fingerprint.as_str()) {
        return Err(VR_DOWNLOAD_STALE);
    }
    if temporary_fingerprint.as_deref() == Some(placeholder_fingerprint.as_str()) {
        persist_cleanup_mutation_phase(
            persistence_path,
            recovery,
            selected_index,
    ...
    clear_cleanup_mutation(
        persistence_path,
        recovery,
        selected_index,
        CleanupFileState::Deleted,
    )
}

#[cfg(target_os = "macos")]
fn reconcile_macos_cleanup_mutation(
    persistence_path: &Path,
    recovery: &mut CleanupRecovery,
    selected_index: usize,
) -> Result<(), &'static str> {
    ...
    let expected_fingerprint = recovery.record.fingerprints[selected_index].clone();
    let temporary_path = mutation.temporary_path;
    if mutation.phase == CleanupMutationPhase::TemporaryCreationPrepared {
        if macos_cleanup_path_fingerprint(&target)?.as_deref()
            != Some(expected_fingerprint.as_str())
            || macos_cleanup_path_fingerprint(&temporary_path)?.is_some()
        {
            return Err(VR_DOWNLOAD_STALE);
        }
        return clear_cleanup_mutation(
            persistence_path,
            recovery,
            selected_index,
    ...
    if matches!(
        ...
        return finish_reconciled_macos_deletion(persistence_path, recovery, selected_index);
    }

    let target_fingerprint = macos_cleanup_path_fingerprint(&target)?;
    let temporary_fingerprint = macos_cleanup_path_fingerprint(&temporary_path)?;
    match (
        target_fingerprint.as_deref(),
        temporary_fingerprint.as_deref(),
    ) {
        (Some(target_identity), Some(temporary_identity))
            if target_identity == expected_fingerprint
                && temporary_identity == placeholder_fingerprint =>
        {
            remove_exact_macos_cleanup_path(&temporary_path, &placeholder_fingerprint)?;
            clear_cleanup_mutation(
                persistence_path,
        ...
        (Some(target_identity), Some(temporary_identity))
            if target_identity == placeholder_fingerprint
                && temporary_identity == expected_fingerprint =>
        {
            macos_rename_exact(&target, &temporary_path, RENAME_SWAP)
                .map_err(|_| VR_DOWNLOAD_CLEANUP_FAILED)?;
            remove_exact_macos_cleanup_path(&temporary_path, &placeholder_fingerprint)?;
        ...
        (Some(target_identity), Some(_)) if target_identity == placeholder_fingerprint => {
            macos_rename_exact(&target, &temporary_path, RENAME_SWAP)
                .map_err(|_| VR_DOWNLOAD_CLEANUP_FAILED)?;
            remove_exact_macos_cleanup_path(&temporary_path, &placeholder_fingerprint)?;
            clear_cleanup_mutation(
        ...
        (_, Some(temporary_identity)) if temporary_identity == expected_fingerprint => {
            persist_cleanup_mutation_phase(
                persistence_path,
                recovery,
                selected_index,
            ...
            finish_reconciled_macos_deletion(persistence_path, recovery, selected_index)
        }
        (Some(target_identity), None) if target_identity == placeholder_fingerprint => {
            persist_cleanup_mutation_phase(
                persistence_path,
                recovery,
                selected_index,
        ...
        (Some(target_identity), None) if target_identity == expected_fingerprint => {
            clear_cleanup_mutation(
                persistence_path,
                recovery,
                selected_index,
...
fn reconcile_cleanup_mutations(
    ...
    for selected_index in 0..recovery.mutations.len() {
        if recovery.mutations[selected_index].is_none() {
            continue;
        }
        #[cfg(target_os = "macos")]
...
fn delete_exact_cleanup_file_with(
    ...
    recovery: &mut CleanupRecovery,
    selected_index: usize,
    before_mutation: impl FnOnce() -> io::Result<()>,
    _after_phase: impl FnMut(CleanupMutationPhase) -> io::Result<()>,
) -> Result<(), &'static str> {
    use std::{
        mem::size_of,
    ...
    let target = current_target(&recovery.record, selected_index)?;
    let expected_fingerprint = &recovery.record.fingerprints[selected_index];
...
fn delete_exact_cleanup_file_with(
    _persistence_path: &Path,
    _recovery: &mut CleanupRecovery,
    _selected_index: usize,
    _before_mutation: impl FnOnce() -> io::Result<()>,
    _after_phase: impl FnMut(CleanupMutationPhase) -> io::Result<()>,
) -> Result<(), &'static str> {
    Err(VR_DOWNLOAD_CLEANUP_FAILED)
}

fn delete_exact_cleanup_file(
    persistence_path: &Path,
    recovery: &mut CleanupRecovery,
    selected_index: usize,
) -> Result<(), &'static str> {
    delete_exact_cleanup_file_with(
        persistence_path,
        recovery,
        selected_index,
        || Ok(()),
...
async fn load_downloads_with_persistence(
    ...
    let mut cleanup_recoveries = match read_cleanup_recoveries(persistence_path) {
        Ok(recoveries) => recoveries,
        Err(error) => {
            state
                .0
    ...
    for recovery in &mut cleanup_recoveries {
        let _ = reconcile_cleanup_mutations(persistence_path, recovery);
        let path = cleanup_recovery_path(persistence_path, &recovery.record)?;
        *recovery = parse_cleanup_recovery(persistence_path, &path)
            .ok_or(VR_DOWNLOAD_PERSISTENCE_FAILED)?;
    }
    ...
    for cleanup in cleanup_recoveries {
        let existing = transfers.iter().position(|transfer| {
            matches!(transfer, StoredTransfer::Valid(record) if record.transfer_id == cleanup.record.transfer_id)
        });
        match existing {
            Some(index) => {
                let StoredTransfer::Valid(record) = &transfers[index] else {
                ...
                if same_transfer_authority(record, &cleanup.record) {
                    transfers[index] = StoredTransfer::Valid(cleanup.record);
                } else {
                    state
                        .0
                        .lock()
            ...
            None if transfers.len() < MAX_PERSISTED_TRANSFERS => {
                transfers.push(StoredTransfer::Valid(cleanup.record));
            }
            None => {}
        }
    }
        ...
        match transfer {
            StoredTransfer::Valid(record) => {
                ...
                rows.extend([
                    ...
                    encoded_paths(&record.current_paths),
                ]);
            }
...
fn list_downloads_with_persistence(
    ...
    let has_durable_recovery = context.transfers.iter().any(|transfer| {
        matches!(transfer, StoredTransfer::Valid(record) if organization_recovery_transfer_ids.contains(&record.transfer_id)
            ...
            || cleanup_recoveries.iter().any(|recovery| same_transfer_authority(record, &recovery.record)))
    });
...
fn cleanup_target_is_owned_elsewhere(
    ...
    for path in terminal_recovery_paths(persistence_path)? {
        ...
        if record.transfer_id != transfer_id
            && transfer_record_owns_path(&record, target).map_err(|_| VR_DOWNLOAD_STALE)?
        {
            return Ok(true);
        }
    }
    for path in cleanup_recovery_paths(persistence_path)? {
        ...
        if recovery.record.transfer_id != transfer_id
            && (transfer_record_owns_path(&recovery.record, target)
                .map_err(|_| VR_DOWNLOAD_STALE)?
                || recovery
                    .mutations
                    .iter()
                    .flatten()
                    .any(|mutation| mutation.temporary_path == target))
        {
            return Ok(true);
        }
    }
...
fn delete_cleanup_files_with(
    state: &VrDownloadState,
    persistence_path: &Path,
    transfer_id: &str,
    mut delete_file: impl FnMut(&Path, &mut CleanupRecovery, usize) -> Result<(), &'static str>,
) -> Result<Vec<String>, &'static str> {
    ...
    let position = context
        ...
        .iter()
        .position(|transfer| {
            matches!(transfer, StoredTransfer::Valid(record) if record.transfer_id == transfer_id && record.state == TransferState::Cleanup)
        })
    ...
    let mut recovery =
        parse_cleanup_recovery(persistence_path, &path).ok_or(VR_DOWNLOAD_PERSISTENCE_FAILED)?;
    if !same_transfer_authority(current, &recovery.record) || !recovery.stopped {
        return Err(VR_DOWNLOAD_CLEANUP_FAILED);
    }
    reconcile_cleanup_mutations(persistence_path, &mut recovery)?;
        ...
        match recovery.files[selected_index] {
            ...
            CleanupFileState::Present => match fs::symlink_metadata(&target) {
                ...
                Ok(metadata) => {
                    if metadata.file_type().is_symlink()
                        || !metadata.is_file()
                        || metadata.len() > recovery.record.selected_files[selected_index].size
                        || fs::canonicalize(&target).ok().as_deref() != Some(target.as_path())
                        || file_fingerprint(&target)?
                            != recovery.record.fingerprints[selected_index]
                    {
                        return Err(VR_DOWNLOAD_STALE);
                    }
                    delete_file(persistence_path, &mut recovery, selected_index)?;
                    match fs::symlink_metadata(&target) {
                        Err(error) if error.kind() == io::ErrorKind::NotFound => {}
                        _ => return Err(VR_DOWNLOAD_STALE),
                    }
    ...
    if let Err(error) = remove_cleanup_recovery(persistence_path, &recovery) {
        context.transfers.insert(position, removed);
        return Err(error);
    }
    Ok(vec![
...
enum CleanupStart {
    ...
    Preparing(Box<CleanupPreparation>),
}

enum CleanupAuthorityResult {
    Durable,
    Restored(&'static str),
    Retained(&'static str),
}
...
struct CleanupRestoreRequirements {
    unpause: bool,
    completion_monitor: bool,
}

fn cleanup_restore_requirements(
    preparation: &CleanupPreparation,
    paused_by_cleanup: bool,
) -> CleanupRestoreRequirements {
    CleanupRestoreRequirements {
        unpause: paused_by_cleanup && preparation.handle_was_paused == Some(false),
        completion_monitor: preparation.previous_record.state.is_active()
            && preparation.handle_was_paused.is_some(),
    }
}

fn mark_cleanup_preparing(
    record: &mut TransferRecord,
    session: Option<Arc<Session>>,
    handle_was_paused: Option<bool>,
) -> CleanupPreparation {
    let previous_record = record.clone();
    record.pending_action = Some(TransferAction::Cancel);
    record.handle_generation = record.handle_generation.wrapping_add(1);
    CleanupPreparation {
        previous_record,
        session,
        ...
        handle_was_paused,
        preparation_generation: record.handle_generation,
    }
}

fn reserve_cleanup_operation(
    context: &mut VrDownloadContext,
    transfer_id: &str,
) -> Result<(), &'static str> {
...
fn release_cleanup_operation(context: &mut VrDownloadContext, transfer_id: &str) {
    if context.cleanup_transfer_id.as_deref() == Some(transfer_id) {
        context.cleanup_transfer_id = None;
    }
}

fn begin_cleanup_preparation(
    state: &VrDownloadState,
    persistence_path: &Path,
    transfer_id: &str,
) -> Result<CleanupStart, &'static str> {
    let mut context = state.0.lock().map_err(|_| VR_DOWNLOAD_CLEANUP_FAILED)?;
    let position = context
        .transfers
        .iter()
        .position(|transfer| matches!(transfer, StoredTransfer::Valid(record) if record.transfer_id == transfer_id))
        .ok_or(VR_DOWNLOAD_STALE)?;
    let StoredTransfer::Valid(record) = &context.transfers[position] else {
        return Err(VR_DOWNLOAD_STALE);
    };
    if record.state == TransferState::Cleanup {
        return Ok(CleanupStart::Existing {
    ...
    if !matches!(
        ...
        || record.terminal_recovery_generation.is_some()
        || record.pending_action.is_some()
    {
        return Err(VR_DOWNLOAD_ACTION_INVALID);
    }
    let previous_record = record.clone();
    validate_cleanup_boundary_data(&previous_record)?;
    for selected_index in 0..previous_record.selected_files.len() {
        let target = current_target(&previous_record, selected_index)?;
        cleanup_target_state(&previous_record, selected_index)?;
        if cleanup_target_is_owned_elsewhere(&context, persistence_path, transfer_id, &target)? {
            return Err(VR_DOWNLOAD_STALE);
        }
    }
    let session = context.session.clone();
    let handle_was_paused = previous_record
        .handle
        .as_ref()
        .map(|handle| handle.is_paused());
    let record = match &mut context.transfers[position] {
        StoredTransfer::Valid(record) => record,
        StoredTransfer::Corrupt(_) => unreachable!(),
    };
    Ok(CleanupStart::Preparing(Box::new(mark_cleanup_preparing(
        record,
        session,
        handle_was_paused,
    ))))
...
fn restore_cleanup_record(
    ...
    let record =
        find_valid_record_mut(&mut context.transfers, transfer_id).ok_or(VR_DOWNLOAD_STALE)?;
    if record.pending_action != Some(TransferAction::Cancel)
        || record.handle_generation != preparation.preparation_generation
    {
        return Err(VR_DOWNLOAD_STALE);
    }
    *record = preparation.previous_record.clone();
    Ok(())
}

fn spawn_restored_cleanup_monitor(
    state: &VrDownloadState,
    persistence_path: &Path,
    preparation: &CleanupPreparation,
) {
    if !preparation.previous_record.state.is_active() {
        return;
    }
    let (Some(session), Some(handle)) = (preparation.session.clone(), preparation.handle.clone())
    else {
        return;
    };
    spawn_completion_monitor(
        ...
        session,
        handle,
        preparation.previous_record.transfer_id.clone(),
        preparation.previous_record.handle_generation,
        persistence_path.to_owned(),
    );
}

async fn restore_pre_cleanup_lifecycle(
    state: &VrDownloadState,
    persistence_path: &Path,
    preparation: &CleanupPreparation,
    paused_by_cleanup: bool,
) -> Result<(), &'static str> {
    let requirements = cleanup_restore_requirements(preparation, paused_by_cleanup);
    if requirements.unpause {
        let (Some(session), Some(handle)) =
            (preparation.session.as_ref(), preparation.handle.as_ref())
        else {
            return Err(VR_DOWNLOAD_CLEANUP_FAILED);
        };
        if session.unpause(handle).await.is_err() {
            let terminal_saved = {
                let mut context = state.0.lock().map_err(|_| VR_DOWNLOAD_CLEANUP_FAILED)?;
                restore_cleanup_record(
                    &mut context,
                    &preparation.previous_record.transfer_id,
                    preparation,
                )?;
                finalize_monitored_transfer_with(
                    &mut context,
    ...
    if requirements.completion_monitor {
        spawn_restored_cleanup_monitor(state, persistence_path, preparation);
    }
    Ok(())
}

fn establish_cleanup_authority_with(
    context: &mut VrDownloadContext,
    persistence_path: &Path,
    preparation: &CleanupPreparation,
    retain_validation_failure: bool,
...
) -> CleanupAuthorityResult {
    let transfer_id = &preparation.previous_record.transfer_id;
    let position = match context.transfers.iter().position(|transfer| {
        matches!(transfer, StoredTransfer::Valid(record) if record.transfer_id == *transfer_id)
    }) {
        Some(position) => position,
        None => return CleanupAuthorityResult::Restored(VR_DOWNLOAD_STALE),
    };
    let current = match &context.transfers[position] {
        StoredTransfer::Valid(record)
            if record.pending_action == Some(TransferAction::Cancel)
                && record.handle_generation == preparation.preparation_generation =>
        {
        ...
        StoredTransfer::Valid(_) | StoredTransfer::Corrupt(_) => {
            return CleanupAuthorityResult::Restored(VR_DOWNLOAD_STALE)
        }
    };
    let validated_files = (0..current.selected_files.len())
        .map(|index| {
            let target = current_target(&current, index)?;
            let file = cleanup_target_state(&current, index)?;
            if cleanup_target_is_owned_elsewhere(context, persistence_path, transfer_id, &target)? {
                return Err(VR_DOWNLOAD_STALE);
            }
            Ok(file)
    ...
    let (files, retained_error) = match validated_files {
        Ok(files) => (files, None),
        Err(error) if retain_validation_failure => (
            (0..current.selected_files.len())
                .map(|index| match current_target(&current, index) {
                    Ok(target) => match fs::symlink_metadata(target) {
                        Err(error) if error.kind() == io::ErrorKind::NotFound => {
                            CleanupFileState::AbsentBeforeCleanup
                        }
                        _ => CleanupFileState::Present,
                    },
                    Err(_) => CleanupFileState::Present,
                })
                .collect(),
            Some(error),
        ),
        Err(error) => return CleanupAuthorityResult::Restored(error),
    };
    let mut cleanup_record = current.clone();
    cleanup_record.state = TransferState::Cleanup;
    cleanup_record.pending_action = None;
    let recovery = CleanupRecovery {
        record: cleanup_record.clone(),
        mutations: vec![None; files.len()],
        files,
        stopped: preparation.handle.is_none(),
    };
    if let Err(error) = persist_recovery(persistence_path, &recovery) {
        return CleanupAuthorityResult::Restored(error);
    }
    ...
    if let Err(error) = persist_primary(persistence_path, &context.transfers) {
        if retained_error.is_some() {
            CleanupAuthorityResult::Retained(error)
        } else if remove_recovery(persistence_path, &recovery).is_ok() {
            context.transfers[position] = StoredTransfer::Valid(current);
            CleanupAuthorityResult::Restored(error)
        } else {
            CleanupAuthorityResult::Retained(error)
        }
    } else if let Some(error) = retained_error {
        CleanupAuthorityResult::Retained(error)
    } else {
        CleanupAuthorityResult::Durable
    }
}

async fn stop_cleanup_transfer(
    state: &VrDownloadState,
    persistence_path: &Path,
    transfer_id: &str,
    session: Option<&Arc<Session>>,
    ...
    record.handle = None;
    let path = cleanup_recovery_path(persistence_path, record)?;
    let mut recovery =
        parse_cleanup_recovery(persistence_path, &path).ok_or(VR_DOWNLOAD_PERSISTENCE_FAILED)?;
    if !same_transfer_authority(record, &recovery.record) {
        return Err(VR_DOWNLOAD_STALE);
    }
    if !recovery.stopped {
...
async fn delete_transfer_files_reserved(
    state: &VrDownloadState,
    persistence_path: &Path,
    transfer_id: &str,
) -> Result<Vec<String>, &'static str> {
    let start = begin_cleanup_preparation(state, persistence_path, transfer_id)?;
    let (session, handle) = match start {
        CleanupStart::Existing { session, handle } => (session, handle),
        CleanupStart::Preparing(preparation) => {
            let pause_required = preparation.handle_was_paused == Some(false);
            if pause_required {
                let (Some(session), Some(handle)) =
                    (preparation.session.as_ref(), preparation.handle.as_ref())
                else {
                ...
                if session.pause(handle).await.is_err() {
                    restore_pre_cleanup_lifecycle(state, persistence_path, &preparation, false)
                        .await?;
                    return Err(VR_DOWNLOAD_CLEANUP_FAILED);
                }
            }
            let authority = {
                let mut context = state.0.lock().map_err(|_| VR_DOWNLOAD_CLEANUP_FAILED)?;
                establish_cleanup_authority_with(
                    &mut context,
                    persistence_path,
                    &preparation,
                    pause_required,
            ...
            match authority {
                ...
                CleanupAuthorityResult::Retained(error) => {
                    stop_cleanup_transfer(
                        state,
                        persistence_path,
                        transfer_id,
                        preparation.session.as_ref(),
                        preparation.handle.as_ref(),
                    )
                    .await?;
                    return Err(error);
                }
            ...
            (preparation.session, preparation.handle)
        }
    };
    ...
    .await?;
    delete_cleanup_files_with(
        state,
        persistence_path,
        transfer_id,
        delete_exact_cleanup_file,
    )
}

async fn delete_transfer_files(
    state: &VrDownloadState,
    persistence_path: &Path,
    transfer_id: &str,
) -> Result<Vec<String>, &'static str> {
    {
        let mut context = state.0.lock().map_err(|_| VR_DOWNLOAD_CLEANUP_FAILED)?;
        reserve_cleanup_operation(&mut context, transfer_id)?;
    }
    let result = delete_transfer_files_reserved(state, persistence_path, transfer_id).await;
    if let Ok(mut context) = state.0.lock() {
        release_cleanup_operation(&mut context, transfer_id);
    }
    result
}

pub async fn cancel_or_delete_download(
    state: &VrDownloadState,
    persistence_path: &Path,
    transfer_id: &str,
    choice: &str,
    ...
    match choice {
        "keep-files" => {
            cancel_download(state, persistence_path, transfer_id).await?;
            Ok(Vec::new())
        }
        "delete-files" => delete_transfer_files(state, persistence_path, transfer_id).await,
        _ => Err(VR_DOWNLOAD_ACTION_INVALID),
    }
}
...
mod tests {
    ...
    fn organization_recovery_loading_is_category_isolated() {
        ...
        assert_eq!(rows[1], "vr");
        assert_eq!(rows[16], "adult");
        assert_eq!(rows[31], "movie");
        assert_eq!(rows[46], "tv");
    }
    ...
    #[test]
    fn cleanup_recovery_retries_only_exact_remaining_files_after_restart() {
        let fixture = FilesystemFixture::new();
        let destination = fixture.path.join("Movies");
        ...
        let recovery = CleanupRecovery {
            record: record.clone(),
            files: vec![CleanupFileState::Present, CleanupFileState::Present],
            mutations: vec![None, None],
            stopped: true,
        };
        write_cleanup_recovery(&persistence_path, &recovery)
            .expect("cleanup authority must persist");
        assert!(parse_cleanup_recovery(
            &persistence_path,
            &cleanup_recovery_path(&persistence_path, &recovery.record).unwrap()
        )
        .is_some());
        ...
        assert_eq!(
            delete_cleanup_files_with(
                ...
                |_, recovery, index| {
                    ...
                    fs::remove_file(current_target(&recovery.record, index)?)
                        .map_err(|_| VR_DOWNLOAD_CLEANUP_FAILED)
                }
            ),
            Err(VR_DOWNLOAD_CLEANUP_FAILED)
        );
        assert!(!destination.join("Provider/Exact Movie.mkv").exists());
        assert_eq!(
            fs::read(destination.join("Provider/notes.txt")).expect("remaining file must survive"),
            vec![b'b'; 3]
        );
        let interrupted = parse_cleanup_recovery(
            &persistence_path,
            &cleanup_recovery_path(&persistence_path, &recovery.record)
                .expect("recovery path must resolve"),
        )
        .expect("interrupted recovery must remain valid");
        assert_eq!(
            interrupted.files,
            vec![CleanupFileState::Deleted, CleanupFileState::Present]
        );
        assert_eq!(
            encoded_boundary_segments(&interrupted.record).expect("boundary data must encode"),
            "2:5:0708"
        );

        let restarted = VrDownloadState::default();
        configure_movie_download_folder(&restarted, Some(destination.clone()))
            .expect("current Movies folder must configure");
        let rows = tauri::async_runtime::block_on(load_downloads(
            &restarted,
            &persistence_path,
            &fixture.path.join("session"),
        ...
        assert_eq!(
            ...
            .expect("restart retry must finish"),
            vec!["movie", "true"]
        );
        assert!(!destination.join("Provider/notes.txt").exists());
        ...
        assert!(cleanup_recovery_paths(&persistence_path)
            .expect("cleanup directory must remain readable")
            .is_empty());
    }

    #[test]
    fn cleanup_rejects_replaced_and_other_transfer_owned_paths_without_dispatch() {
        let fixture = FilesystemFixture::new();
        ...
        let state = VrDownloadState::default();
        {
            ...
            context.transfers_loaded = true;
            context.persistence_path = Some(persistence_path.clone());
            context.transfers.extend([
                StoredTransfer::Valid(record.clone()),
                StoredTransfer::Valid(owner),
            ]);
        }
        let dispatches = Cell::new(0);
        assert_eq!(
            delete_cleanup_files_with(&state, &persistence_path, &transfer_id, |_, _, _| {
        ...
        assert_eq!(
            fs::read(destination.join("Movie  A.mp4")).unwrap(),
            vec![b'a'; 5]
        );

        state.0.lock().expect("state must lock").transfers.pop();
        fs::remove_file(destination.join("Movie  A.mp4")).unwrap();
        fs::write(destination.join("Movie  A.mp4"), b"other").unwrap();
        assert_eq!(
            delete_cleanup_files_with(&state, &persistence_path, &transfer_id, |_, _, _| {
                dispatches.set(dispatches.get() + 1);
                Ok(())
    ...
    #[test]
    fn cancelled_movie_tv_adult_and_vr_cleanup_delete_selected_members_only() {
        let fixture = FilesystemFixture::new();
        for category in [
            TransferCategory::Movie,
            TransferCategory::Tv,
            ...
            let destination = fixture.path.join(category.as_str());
            fs::create_dir(&destination).expect("category destination must exist");
            let destination = fs::canonicalize(destination).expect("destination must canonicalize");
            let source = match category {
                TransferCategory::Movie => movie_organization_source(
                    "Exact Movie",
                    Some("1999-04-19"),
                    "Exact Movie",
                    &[("Provider/media.mkv", 5), ("Provider/notes.txt", 3)],
                    &[0, 1],
                ),
                TransferCategory::Tv => tv_download_source(
                    &[("Provider/media.mkv", 5), ("Provider/notes.txt", 3)],
                    &[0, 1],
                ),
                TransferCategory::Adult => persistable_adult_fixture_source(),
            ...
            let mut record =
                completed_organization_record_for_category(category, &destination, source);
            record.state = TransferState::Cancelled;
            let selected_paths = record
                .current_paths
                .iter()
            ...
            let persistence_path = fixture
                ...
                .join(format!("{}-downloads", category.as_str()));
            write_persisted_transfers(&persistence_path, &[StoredTransfer::Valid(record.clone())])
                .expect("cancelled authority must persist");
            let state = VrDownloadState::default();
            {
                let mut context = state.0.lock().expect("state must lock");
                match category {
                    ...
                    TransferCategory::Tv => context.tv_future_folder = Some(destination.clone()),
                    TransferCategory::Vr => context.future_folder = Some(destination.clone()),
                }
                context.transfers_loaded = true;
                context.persistence_path = Some(persistence_path.clone());
                context.transfers.push(StoredTransfer::Valid(record));
            ...
            assert_eq!(
                tauri::async_runtime::block_on(delete_transfer_files(
                    &state,
                    &persistence_path,
                    &transfer_id,
                ))
                .expect("explicit cleanup must finish"),
                vec![category.as_str(), "true"]
            );
            assert!(selected_paths.iter().all(|path| !path.exists()));
            assert_eq!(fs::read(&unrelated).unwrap(), b"unrelated");
            assert!(destination.is_dir(), "cleanup must not remove directories");
        }
    }

    #[test]
    fn failed_cleanup_authority_restores_the_exact_cancelled_row_and_all_files() {
        let fixture = FilesystemFixture::new();
        let destination = fixture.path.join("VR");
        fs::create_dir(&destination).expect("destination must exist");
        let destination = fs::canonicalize(destination).expect("destination must canonicalize");
        let mut record = completed_organization_record_for_category(
            TransferCategory::Vr,
        ...
        record.state = TransferState::Cancelled;
        record.handle_generation = 17;
        let transfer_id = record.transfer_id.clone();
        let selected_path = current_target(&record, 0).expect("selected path must resolve");
        let blocked_parent = fixture.path.join("blocked-parent");
        fs::write(&blocked_parent, b"not a directory").expect("blocking file must exist");
        let persistence_path = blocked_parent.join("downloads");
        let state = VrDownloadState::default();
        {
            let mut context = state.0.lock().expect("state must lock");
        ...
        let restored = context
            ...
            .expect("cancelled row must remain");
        assert_eq!(restored.state, TransferState::Cancelled);
        assert_eq!(restored.handle_generation, 17);
        assert_eq!(restored.pending_action, None);
        assert_eq!(fs::read(selected_path).unwrap(), vec![b'a'; 5]);
    }

    #[test]
    fn pause_failures_restore_queued_downloading_and_paused_lifecycles_exactly() {
        let fixture = FilesystemFixture::new();
        for lifecycle in [
            TransferState::Queued,
            TransferState::Downloading,
            ...
            let destination = fixture.path.join(format!("pause-failure-{label}"));
            fs::create_dir(&destination).expect("destination must exist");
            let destination = fs::canonicalize(destination).expect("destination must canonicalize");
            let mut record = completed_organization_record_for_category(
                TransferCategory::Vr,
            ...
            record.handle_generation = 41;
            record.downloaded_bytes = 3;
            let transfer_id = record.transfer_id.clone();
            let target = current_target(&record, 0).expect("target must resolve");
            let persistence_path = fixture
                .path
                .join(format!("pause-failure-{label}.downloads"));
            let expected = encode_transfer(&record).expect("prior lifecycle must encode");
            write_persisted_transfers(&persistence_path, &[StoredTransfer::Valid(record.clone())])
                .expect("prior lifecycle must persist");
            let state = VrDownloadState::default();
            {
                let mut context = state.0.lock().expect("state must lock");
                context.transfers_loaded = true;
            ...
            let preparation = {
                let mut context = state.0.lock().expect("state must lock");
                let record = find_valid_record_mut(&mut context.transfers, &transfer_id)
                    .expect("transfer must exist");
                mark_cleanup_preparing(record, None, Some(false))
            };
            assert_eq!(
                cleanup_restore_requirements(&preparation, false),
                CleanupRestoreRequirements {
                    unpause: false,
                    completion_monitor: true,
                }
            );
            tauri::async_runtime::block_on(restore_pre_cleanup_lifecycle(
                &state,
                &persistence_path,
                &preparation,
                false,
            ))
            .expect("pause failure must restore the prior lifecycle");
            {
                let mut context = state.0.lock().expect("state must lock");
                let restored = find_valid_record_mut(&mut context.transfers, &transfer_id)
                    .expect("restored transfer must remain");
                assert_eq!(restored.state, lifecycle);
                assert_eq!(restored.handle_generation, 41);
                assert_eq!(restored.pending_action, None);
                assert_eq!(restored.downloaded_bytes, 3);
            }

            assert_eq!(transfer_snapshots(&state), vec![expected.clone()]);
            assert_eq!(fs::read(&target).expect("media must remain"), vec![b'a'; 5]);
            assert!(cleanup_recovery_paths(&persistence_path)
                .expect("cleanup recovery directory must be readable")
                .is_empty());
            let restarted_transfers = read_persisted_transfers(&persistence_path)
                .expect("prior durable authority must remain");
            let [StoredTransfer::Valid(restarted)] = restarted_transfers.as_slice() else {
                panic!("restarted transfer must remain valid");
            };
            assert_eq!(
                encode_transfer(restarted).expect("restarted lifecycle must encode"),
                expected
            );
        }
    }
    ...
    #[test]
    fn stale_after_pause_retains_cleanup_for_queued_downloading_and_paused_lifecycles() {
        let fixture = FilesystemFixture::new();
        for lifecycle in [
            TransferState::Queued,
            TransferState::Downloading,
            ...
            let label = lifecycle.as_str();
            let destination = fixture.path.join(format!("stale-after-pause-{label}"));
            fs::create_dir(&destination).expect("destination must exist");
            let destination = fs::canonicalize(destination).expect("destination must canonicalize");
            let mut record = completed_organization_record_for_category(
                TransferCategory::Vr,
            ...
            record.state = lifecycle;
            record.handle_generation = 73;
            let transfer_id = record.transfer_id.clone();
            let target = current_target(&record, 0).expect("target must resolve");
            let persistence_path = destination.join(".downloads");
            write_persisted_transfers(&persistence_path, &[StoredTransfer::Valid(record.clone())])
                .expect("prior lifecycle must persist");
            let state = VrDownloadState::default();
            {
            ...
            fs::remove_file(&target).expect("validated file must be replaceable");
            fs::write(&target, b"other").expect("replacement must exist");
            let authority = {
                let mut context = state.0.lock().expect("state must lock");
                establish_cleanup_authority_with(
                    &mut context,
            ...
            assert!(matches!(
                authority,
                CleanupAuthorityResult::Retained(VR_DOWNLOAD_STALE)
            ));
            assert_eq!(
                cleanup_restore_requirements(&preparation, true),
                CleanupRestoreRequirements {
                    unpause: true,
                    completion_monitor: true,
                }
            ...
            let current_rows = transfer_rows(&state);
            assert_eq!(current_rows[8], "cleanup");
            assert_eq!(current_rows[7], "0");
            {
                let mut context = state.0.lock().expect("state must lock");
                let retained = find_valid_record_mut(&mut context.transfers, &transfer_id)
                    .expect("cleanup transfer must remain");
                assert_eq!(retained.state, TransferState::Cleanup);
                assert_eq!(retained.handle_generation, 74);
                assert_eq!(retained.pending_action, None);
            }

            let restarted = VrDownloadState::default();
            let restarted_rows = tauri::async_runtime::block_on(load_downloads(
                &restarted,
                &persistence_path,
                &fixture
                    .path
                    .join(format!("stale-after-pause-{label}.session")),
                &fixture
                    .path
                    .join(format!("stale-after-pause-{label}.limit")),
            ))
            .expect("durable cleanup authority must load");
            assert_eq!(restarted_rows, current_rows);
            assert_eq!(
                fs::read(&target).expect("restart must retain replacement"),
                b"other"
            );
            let recovery =
                read_cleanup_recoveries(&persistence_path).expect("cleanup recovery must remain");
            assert_eq!(recovery.len(), 1);
            assert_eq!(recovery[0].files, vec![CleanupFileState::Present]);
        }
    }

    #[test]
    fn failed_primary_and_failed_recovery_removal_keep_one_non_running_authority() {
        let fixture = FilesystemFixture::new();
        let destination = fixture.path.join("recovery-removal-failure");
        fs::create_dir(&destination).expect("destination must exist");
        let destination = fs::canonicalize(destination).expect("destination must canonicalize");
        let mut record = completed_organization_record_for_category(
            TransferCategory::Movie,
        ...
        let transfer_id = record.transfer_id.clone();
        let target = current_target(&record, 0).expect("target must resolve");
        let persistence_path = fixture.path.join("recovery-removal-failure.downloads");
        write_persisted_transfers(&persistence_path, &[StoredTransfer::Valid(record.clone())])
            .expect("cancelled authority must persist");
        let state = VrDownloadState::default();
        {
            let mut context = state.0.lock().expect("state must lock");
            context.movie_future_folder = Some(destination);
            context.transfers_loaded = true;
            context.persistence_path = Some(persistence_path.clone());
            context.transfers.push(StoredTransfer::Valid(record));
        }
        let preparation = match begin_cleanup_preparation(&state, &persistence_path, &transfer_id)
            .expect("cleanup preparation must begin")
        {
            CleanupStart::Preparing(preparation) => preparation,
            CleanupStart::Existing { .. } => panic!("cleanup must be new"),
        };
        let authority = {
            let mut context = state.0.lock().expect("state must lock");
            establish_cleanup_authority_with(
        ...
        assert!(matches!(
            authority,
            CleanupAuthorityResult::Retained(VR_DOWNLOAD_PERSISTENCE_FAILED)
        ));
        tauri::async_runtime::block_on(stop_cleanup_transfer(
            &state,
            &persistence_path,
            &transfer_id,
            None,
        ...
        .expect("durable recovery must already describe a stopped transfer");
        let current_rows = transfer_rows(&state);
        assert_eq!(current_rows[8], "cleanup");
        assert_eq!(current_rows[7], "0");
        assert_eq!(fs::read(&target).expect("media must remain"), vec![b'a'; 5]);
        ...
        let restarted = VrDownloadState::default();
        configure_movie_download_folder(
            &restarted,
            Some(preparation.previous_record.destination.clone()),
        )
        .expect("current Movies folder must configure after restart");
        let restarted_rows = tauri::async_runtime::block_on(load_downloads(
            &restarted,
            &persistence_path,
            &fixture.path.join("recovery-removal-failure.session"),
            &fixture.path.join("recovery-removal-failure.limit"),
        ))
        .expect("recovery must override the old active primary authority");
        assert_eq!(restarted_rows, current_rows);
        assert_eq!(
            fs::read(&target).expect("restart must retain media"),
            vec![b'a'; 5]
        );
    }

    #[test]
    fn final_deletion_race_preserves_replacement_and_cleanup_authority() {
        let fixture = FilesystemFixture::new();
        let destination = fixture.path.join("final-race");
        fs::create_dir(&destination).expect("destination must exist");
        let destination = fs::canonicalize(destination).expect("destination must canonicalize");
        let mut record = completed_organization_record_for_category(
            TransferCategory::Vr,
        ...
        let target = current_target(&record, 0).expect("target must resolve");
        let temporary_path =
            cleanup_temporary_path(&record, 0).expect("temporary path must resolve");
        let displaced = destination.join("displaced-original.mp4");
        let persistence_path = fixture.path.join("final-race.downloads");
        let recovery = CleanupRecovery {
            record: record.clone(),
            files: vec![CleanupFileState::Present],
            mutations: vec![None],
        ...
        assert_eq!(
            delete_cleanup_files_with(
                ...
                &persistence_path,
                &transfer_id,
                |persistence_path, recovery, selected_index| {
                    delete_exact_cleanup_file_with(
                        persistence_path,
                        recovery,
                        selected_index,
        ...
        let durable =
            read_cleanup_recoveries(&persistence_path).expect("cleanup authority must remain");
        assert_eq!(durable.len(), 1);
        assert_eq!(durable[0].files, vec![CleanupFileState::Present]);
        assert!(durable[0].mutations.iter().all(Option::is_none));
        assert!(!temporary_path.exists());
        assert!(matches!(
            &read_persisted_transfers(&persistence_path)
                .expect("cleanup row must remain")[0],
            StoredTransfer::Valid(record)
                if record.transfer_id == transfer_id && record.state == TransferState::Cleanup
        ));
    }

    #[cfg(target_os = "macos")]
    #[test]
    fn macos_cleanup_interruption_phases_reconcile_exactly_after_restart() {
        for interruption in [
            CleanupMutationPhase::TemporaryCreated,
            CleanupMutationPhase::Swapped,
            CleanupMutationPhase::ExactDeleted,
            ...
            let fixture = FilesystemFixture::new();
            let destination = fixture.path.join(interruption.as_str());
            fs::create_dir(&destination).expect("destination must exist");
            let destination = fs::canonicalize(destination).expect("destination must canonicalize");
            let mut record = completed_organization_record_for_category(
                TransferCategory::Vr,
            ...
            let temporary_path =
                cleanup_temporary_path(&record, 0).expect("temporary path must resolve");
            let unrelated = destination.join("unrelated.bin");
            fs::write(&unrelated, b"unrelated").expect("unrelated file must exist");
            let persistence_path = fixture.path.join("downloads");
            let recovery = CleanupRecovery {
                record: record.clone(),
                files: vec![CleanupFileState::Present],
                        ...
                        delete_exact_cleanup_file_with(
                            ...
                            |phase| {
                                if phase == interruption {
                                    Err(io::Error::other("deterministic interruption"))
                                } else {
                                    Ok(())
                                }
                            },
            ...
            assert_eq!(current_rows[7], "0");
            assert_eq!(current_rows[8], "cleanup");
            let interrupted = read_cleanup_recoveries(&persistence_path)
                .expect("interrupted cleanup authority must remain");
            assert_eq!(interrupted.len(), 1);
            assert_eq!(
                interrupted[0].mutations[0]
                    .as_ref()
                    .expect("mutation phase must remain")
                    .phase,
                interruption
            );
            ...
            if replacement {
                if target.exists() {
                    fs::remove_file(&target).expect("placeholder must be replaceable");
                }
            ...
            .expect("interrupted cleanup must reload");
            assert_eq!(restarted_rows, current_rows);
            assert_eq!(restarted_rows[7], "0");
            assert_eq!(restarted_rows[8], "cleanup");
            {
                let mut context = restarted.0.lock().expect("restarted state must lock");
                let record = find_valid_record_mut(&mut context.transfers, &transfer_id)
                    .expect("cleanup row must remain");
                assert_eq!(record.state, TransferState::Cleanup);
                assert!(record.handle.is_none());
                assert_eq!(record.pending_action, None);
            }
            assert!(!temporary_path.exists(), "temporary file must reconcile");
            assert_eq!(fs::read(&unrelated).unwrap(), b"unrelated");
            let reconciled = read_cleanup_recoveries(&persistence_path)
                .expect("reconciled cleanup authority must remain");
            assert_eq!(reconciled.len(), 1);
            assert!(reconciled[0].mutations.iter().all(Option::is_none));
            let deleted_before_restart = matches!(
                interruption,
                CleanupMutationPhase::ExactDeleted
                    | CleanupMutationPhase::TemporaryRelocated
                    | CleanupMutationPhase::TemporaryCleanupPrepared
            ...
            if replacement {
                ...
                fs::remove_file(&target).expect("fixture replacement must be removable");
            }
            delete_cleanup_files_with(
                &restarted,
                &persistence_path,
            ...
            .expect("durably reconciled cleanup may complete");
    ...
    #[test]
    fn one_cleanup_reservation_blocks_a_second_transfer_without_mutation() {
        let fixture = FilesystemFixture::new();
        let first_destination = fixture.path.join("first-cleanup");
        let second_destination = fixture.path.join("second-cleanup");
        fs::create_dir(&first_destination).expect("first destination must exist");
        fs::create_dir(&second_destination).expect("second destination must exist");
        let first_destination =
            fs::canonicalize(first_destination).expect("first destination must canonicalize");
        let second_destination =
            fs::canonicalize(second_destination).expect("second destination must canonicalize");
        let mut first = completed_organization_record_for_category(
            TransferCategory::Vr,
            &first_destination,
            persistable_fixture_source(),
        );
        let mut second = completed_organization_record_for_category(
            TransferCategory::Adult,
            &second_destination,
            persistable_adult_fixture_source(),
        );
        first.state = TransferState::Cancelled;
        second.state = TransferState::Cancelled;
        let first_id = first.transfer_id.clone();
        let second_id = second.transfer_id.clone();
        let first_target = current_target(&first, 0).expect("first target must resolve");
        let second_target = current_target(&second, 0).expect("second target must resolve");
        let persistence_path = fixture.path.join("serialized-cleanup.downloads");
        write_persisted_transfers(
            &persistence_path,
            &[
                StoredTransfer::Valid(first.clone()),
                StoredTransfer::Valid(second.clone()),
            ],
        )
        .expect("both rows must persist");
        let state = VrDownloadState::default();
        {
            let mut context = state.0.lock().expect("state must lock");
            context.future_folder = Some(first_destination);
            context.adult_future_folder = Some(second_destination);
            context.transfers_loaded = true;
            context.persistence_path = Some(persistence_path.clone());
            context
                .transfers
                .extend([StoredTransfer::Valid(first), StoredTransfer::Valid(second)]);
            reserve_cleanup_operation(&mut context, &first_id)
                .expect("first cleanup must reserve native state");
        }
        let before_snapshots = transfer_snapshots(&state);
        let before_rows = transfer_rows(&state);
        assert_eq!(
        ...
        assert_eq!(transfer_rows(&state), before_rows);
        assert_eq!(
            fs::read(&first_target).expect("first media must remain"),
            vec![b'a'; 5]
        );
        assert_eq!(
            fs::read(&second_target).expect("second media must remain"),
            vec![b'a'; 5]
        );
        assert!(cleanup_recovery_paths(&persistence_path)
            .expect("cleanup recovery directory must be readable")
        ...
        {
            let mut context = state.0.lock().expect("state must lock");
            release_cleanup_operation(&mut context, &first_id);
        }
        tauri::async_runtime::block_on(delete_transfer_files(
            &state,
            &persistence_path,
        ...
        .expect("second cleanup may proceed after the first reservation ends");
