...
fn finalize_monitored_transfer_with(
    ...
    let mut is_tv_completion = false;
    ...
    if completed && terminal_state_persisted {
        let recovery_removed = !is_tv_completion
...
mod tests {
    ...
    #[test]
    fn tv_completion_never_persists_completed_before_recovery_is_durable() {
        let fixture = FilesystemFixture::new();
        let record = locally_completed_tv_boundary_record(&fixture, "TV — recovery unavailable");
        let transfer_id = record.transfer_id.clone();
        let destination = record.destination.clone();
        let persistence_path = fixture.path.join("downloads");
        ...
        let rows = tauri::async_runtime::block_on(load_downloads(
            &restarted,
            &persistence_path,
            &fixture.path.join("missing-recovery-session"),
            &fixture.path.join("download-limit"),
        ))
        .expect("durable failed primary state must reload without a recovery file");
        assert_eq!(rows[0], transfer_id);
        assert_eq!(rows[1], "tv");
        assert_eq!(rows[6], "7");
        assert_eq!(rows[8], "failed");
    ...
    #[test]
    fn tv_completion_recovery_rejects_changed_identity_without_attaching_a_row() {
        let fixture = FilesystemFixture::new();
        let mut record = locally_completed_tv_boundary_record(&fixture, "TV — changed recovery");
        record.state = TransferState::Failed;
        record.downloaded_bytes = record.selected_total();
        let destination = record.destination.clone();
        let recovery_path = tv_completion_recovery_path(&record);
        ...
        fs::write(
            &recovery_path,
            encoded_tv_completion_recovery(&record)
                .expect("changed recovery fixture must remain encodable"),
        )
        .expect("changed recovery fixture must write");
        assert!(parse_tv_completion_recovery(&recovery_path).is_none());
        ...
        let rows = tauri::async_runtime::block_on(load_downloads(
            &restarted,
            &fixture.path.join("downloads"),
            &fixture.path.join("changed-recovery-session"),
            &fixture.path.join("download-limit"),
        ))
        .expect("an unrelated recovery must not create a global load error");
        assert!(rows.is_empty());
        assert!(restarted
            .0
            .lock()
        ...
        assert_eq!(
            fs::read(destination.join("Folder/特別版  B.mp4"))
                .expect("rejected recovery must preserve selected TV media"),
            b"1234567"
        );
    }

    #[test]
    fn tv_failed_completion_dismiss_failures_preserve_recovery_and_media_until_durable_success() {
        let fixture = FilesystemFixture::new();
        let mut record = locally_completed_tv_boundary_record(&fixture, "TV — dismissal recovery");
        record.state = TransferState::Failed;
        record.downloaded_bytes = record.selected_total();
        let transfer_id = record.transfer_id.clone();
        let destination = record.destination.clone();
        ...
        let recovery_path = tv_completion_recovery_path(&record);
        write_persisted_transfers(&persistence_path, &[StoredTransfer::Valid(record)])
            .expect("failed TV state must persist");
        let persisted = read_persisted_transfers(&persistence_path)
            .expect("failed TV state must remain readable");
        let StoredTransfer::Valid(recovery) = &persisted[0] else {
            panic!("failed TV state must remain valid");
        };
        write_tv_completion_recovery(recovery).expect("TV recovery must persist");
        ...
        fs::remove_file(&persistence_path).expect("primary state must be replaceable");
        ...
        configure_tv_download_folder(&persistence_failed_restart, Some(destination.clone()))
            .expect("restarted TV folder must configure");
        let rows = tauri::async_runtime::block_on(load_downloads(
            &persistence_failed_restart,
            &persistence_path,
            &fixture.path.join("dismiss-persistence-failed-session"),
            &fixture.path.join("download-limit"),
        ))
        .expect("durable recovery must remain visible while primary persistence fails");
        assert_eq!(rows[0], transfer_id);
        assert_eq!(rows[1], "tv");
        assert_eq!(rows[8], "failed");
        assert_eq!(rows[10], "none");
        assert_eq!(rows[12], "false");
        assert!(persistence_failed_restart
            .0
            .lock()
            .expect("state must lock")
            .session
        ...
        fs::remove_dir(&persistence_path).expect("primary path must become available");
        {
            let context = persistence_failed_restart
                ...
                .expect("state must lock");
            write_persisted_transfers(&persistence_path, &context.transfers)
                .expect("recovered failed TV state must persist");
        }
        fs::remove_file(&recovery_path).expect("recovery file must be replaceable");
        ...
        assert!(matches!(
            &read_persisted_transfers(&persistence_path)
                .expect("failed dismissal must restore the primary row")[0],
            StoredTransfer::Valid(record) if record.transfer_id == transfer_id
                && record.state == TransferState::Failed
        ));

        let cleanup_failed_restart = VrDownloadState::default();
        configure_tv_download_folder(&cleanup_failed_restart, Some(destination.clone()))
            .expect("cleanup restart TV folder must configure");
        let rows = tauri::async_runtime::block_on(load_downloads(
            &cleanup_failed_restart,
            &persistence_path,
            &fixture.path.join("dismiss-cleanup-failed-session"),
            &fixture.path.join("download-limit"),
        ))
        .expect("failed dismissal must remain visible after restart");
        assert_eq!(rows[0], transfer_id);
        assert_eq!(rows[8], "failed");
        assert_eq!(rows[12], "false");
        assert_eq!(
            fs::read(destination.join("Folder/特別版  B.mp4"))
                .expect("failed dismissals must preserve selected TV media"),
            b"1234567"
        );
        assert!(!destination.join("Folder/Part  1 — 映画.mkv").exists());
        ...
        {
            let context = cleanup_failed_restart.0.lock().expect("state must lock");
            let StoredTransfer::Valid(record) = &context.transfers[0] else {
                panic!("failed TV state must remain valid");
            };
            write_tv_completion_recovery(record).expect("TV recovery must persist again");
        }
        dismiss_download(&cleanup_failed_restart, &persistence_path, &transfer_id)
            .expect("TV recovery must dismiss after persistence recovers");
        assert!(!recovery_path.exists());
        assert_eq!(
            fs::read(destination.join("Folder/特別版  B.mp4"))
                .expect("successful dismiss must preserve selected TV media"),
            b"1234567"
        );
        ...
        for restart_index in 0..2 {
            ...
            let rows = tauri::async_runtime::block_on(load_downloads(
                ...
                &fixture
                    .path
                    .join(format!("dismissed-tv-recovery-session-{restart_index}")),
                &fixture.path.join("download-limit"),
            ))
            .expect("dismissed TV state must reload");
            assert!(rows.is_empty(), "dismissed TV recovery recreated its row");
            assert_eq!(
                fs::read(destination.join("Folder/特別版  B.mp4"))
                    .expect("dismissed restart must preserve selected TV media"),
                b"1234567"
            );
            assert!(!destination.join("Folder/Part  1 — 映画.mkv").exists());
        }
