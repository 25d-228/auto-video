...
fn valid_text_request(request: &ProviderTextRequest) -> bool {
    ...
    match (request.method, host) {
        ...
        (ProviderMethod::Post, "graphql.anilist.co") => path.is_empty(),
...
mod tests {
    ...
    #[test]
    fn production_provider_urls_pass_the_exact_transport_validators() {
        for (method, url) in [
            (
                ProviderMethod::Get,
                "https://api.themoviedb.org/3/search/movie?query=Exact",
            ),
            (
                ProviderMethod::Get,
                "https://api.tvmaze.com/lookup/shows?imdb=tt1234567",
            ),
            (ProviderMethod::Post, "https://graphql.anilist.co/"),
            (
                ProviderMethod::Get,
                "https://r18.dev/videos/vod/movies/detail/-/id=exact/json",
            ),
            (
                ProviderMethod::Get,
                "https://www.mgstage.com/product/product_detail/EXACT/",
            ),
            (
                ProviderMethod::Get,
                "https://www.javdatabase.com/movies/exact/",
            ),
        ] {
            assert!(valid_text_request(&ProviderTextRequest {
                method,
                url: url.to_owned(),
                accept: "application/json",
                referer: None,
        ...
        for (url, referer) in [
            (
                "https://image.tmdb.org/t/p/w500/exact.jpg",
            ...
            (
                "https://s1.anilist.co/file/anilistcdn/media/anime/cover/large/exact.jpg",
                "https://anilist.co/",
            ),
            (
                "https://pics.dmm.co.jp/digital/video/exact/exactps.jpg",
                "https://www.dmm.co.jp/",
            ),
            (
                "https://image.mgstage.com/images/exact.jpg",
                "https://www.mgstage.com/",
            ),
            (
                "https://www.javdatabase.com/covers/exact.jpg",
                "https://www.javdatabase.com/",
            ),
        ] {
            assert!(valid_image_request(&ProviderImageRequest {
                url: url.to_owned(),
                referer: referer.to_owned(),
                cookie: String::new(),
            }));
        ...
        assert!(!valid_image_request(&ProviderImageRequest {
            url: "https://static.tvmaze.com/".to_owned(),
            referer: "https://www.tvmaze.com/".to_owned(),
            cookie: String::new(),
        }));
        assert!(!valid_text_request(&ProviderTextRequest {
            method: ProviderMethod::Post,
    ...
    #[test]
    fn tvmaze_imdb_fallback_accepts_a_different_script_with_the_exact_imdb_identity() {
        let authority = LibraryItemAuthority {
            category: LibraryCategory::Tv,
            identity: "exact-imdb-tv".to_owned(),
            local_title: "孤独のグルメ".to_owned(),
            year: None,
            code: None,
        };
        let image = tvmaze_image(
            r#"{"name":"Kodoku no Gurume","externals":{"imdb":"tt2147413"},"image":{"original":"https://static.tvmaze.com/uploads/images/original_untouched/1/2.jpg"}}"#,
            &authority,
            Some("tt2147413"),
        )
        .expect("an exact IMDb lookup establishes TVMaze identity");

        assert_eq!(
            image.as_deref(),
            Some("https://static.tvmaze.com/uploads/images/original_untouched/1/2.jpg")
        );
        assert_eq!(
            tvmaze_image(
                r#"{"name":"Kodoku no Gurume","externals":{"imdb":"tt9999999"},"image":{"original":"https://static.tvmaze.com/uploads/images/original_untouched/1/2.jpg"}}"#,
                &authority,
                Some("tt2147413"),
            ),
            Err(CoverDocumentError::Conflicting)
    ...
    #[test]
    fn tv_cover_fallback_uses_exact_imdb_authority_across_different_scripts() {
        let authority = LibraryItemAuthority {
            category: LibraryCategory::Tv,
            identity: "different-script-tv".to_owned(),
            local_title: "孤独のグルメ".to_owned(),
            year: None,
            code: None,
        };
        ...
        let (presentation, bytes, cache_cover) = resolve_movie_or_tv_presentation(
            ...
            &mut |request| {
                ...
                requests.push(request.url.clone());
                if request.url.contains("search/tv") {
                    Ok(r#"{"results":[{"id":91,"name":"孤独のグルメ","first_air_date":"2012-01-04"}]}"#.to_owned())
                } else if request.url.contains("/tv/91?") {
                    Ok(r#"{"id":91,"name":"孤独のグルメ","first_air_date":"2012-01-04","genres":[],"credits":{"cast":[]},"external_ids":{"imdb_id":"tt2147413"},"poster_path":null}"#.to_owned())
                } else if request.url.contains("lookup/shows") {
                    Ok(r#"{"name":"Kodoku no Gurume","externals":{"imdb":"tt2147413"},"image":{"medium":"https://static.tvmaze.com/uploads/images/medium_portrait/1/2.jpg"}}"#.to_owned())
                } else {
                    panic!("an exact IMDb result must stop later TV fallbacks")
                }
            },
            &mut |request| {
                assert!(valid_image_request(request));
                Ok(jpeg(400, 600))
            },
        )
        .expect("the exact IMDb fallback must resolve");
