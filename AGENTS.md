...
A more specific higher-priority instruction overrides a general rule only for its named action and current task. Issue requirements override general repository rules, and applicable nested rules override this root contract for their paths. Do not reject an explicit current human or orchestrator instruction because a lower-priority rule contains a general prohibition. When no rule specifies a detail, preserve the conventions of the code being changed.

## Scope and design

- Build only what the issue requires. Do not add speculative options, hooks, wrappers, abstractions, files, dependencies, cleanup, documentation, or infrastructure.
...
- Match repository formatters and nearby maintained source. Do not compress or reformat unrelated code, or introduce unexplained constants, dead code, unused exports, or unreachable branches.
...
- Do not weaken, skip, ignore, delete, or change a test only to make the implementation pass. Mock an external boundary only when necessary and consistent with surrounding tests.
...
- Do not launch or interact with the application unless the task explicitly requires it. Do not use GUI, browser, Finder, AppleScript, Accessibility APIs, simulated-input automation, or persistent services for validation.
