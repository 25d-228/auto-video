...
type LibraryDetailsContext = {
  ...
  presentation: LibraryPresentation | null;
  presentationState:
    | "explicit"
    | "automatic"
    | "local-only"
...
function LibraryPresentationSurface({
  ...
  detailsUpdate.current = onDetailsUpdate;
  reportRatio.current = onRatioChange;
  ...
  const presentation =
    ...
    (automaticState.status === "ready"
      ? automaticState.presentation
      : request === null
        ? localOnlyLibraryPresentation
        : null);
  const presentationState: LibraryDetailsContext["presentationState"] =
    explicitPresentation !== null
      ? "explicit"
      : request === null
        ? "local-only"
        : automaticState.status === "loading"
          ? "loading"
          : automaticState.status === "error"
            ? "unavailable"
            : automaticState.presentation.state === "automatic"
              ? "automatic"
              : "local-only";
  const stateLabel =
    presentationState === "explicit"
      ? "Explicit match · TMDB"
  ...
  const automaticFacts =
    presentationState === "automatic" && presentation !== null
      ? [
          presentation.title !== null && presentation.title !== localTitle
            ? presentation.title
            : null,
          [
            presentation.date,
  ...
  const placeholderMessage =
    presentationState === "loading"
      ...
      : presentationState === "unavailable"
        ? "Presentation unavailable"
        : automaticState.status === "ready" &&
            automaticState.coverStatus === "loading"
          ? "Loading cover"
          : "Cover unavailable";
  const detailsContext: LibraryDetailsContext = {
    category,
    code,
    coverUrl,
    localTitle,
  ...
  const detailsContextRef = useRef(detailsContext);
  detailsContextRef.current = detailsContext;
  const detailsRevision = JSON.stringify([
    presentationState,
    coverUrl,
    presentation,
  ]);
  const retainedAspectRatio = presentation?.aspectRatio ?? null;
  ...
  useEffect(() => {
    detailsUpdate.current(detailsContextRef.current);
  }, [detailsRevision]);

  useEffect(() => {
    if (retainedAspectRatio !== null) {
      reportRatio.current(retainedAspectRatio);
    }
  }, [retainedAspectRatio]);
        ...
        {coverUrl === null ? (
          ...
          <img
            ...
            onLoad={(event) => {
              const { naturalHeight, naturalWidth } = event.currentTarget;
              if (naturalHeight > 0 && naturalWidth > 0) {
                reportRatio.current(naturalWidth / naturalHeight);
              }
            }}
            src={coverUrl}
          />
                    ...
                    <p className="card-eyebrow">
                      {context.presentationState === "explicit"
                        ...
                        : context.presentationState === "automatic"
                          ...
                          : context.presentationState === "loading"
                            ? "Loading presentation"
                            : context.presentationState === "unavailable"
                              ? "Presentation unavailable"
                              : "Local only"}
                    </p>
                ...
                <Dialog.Description className="movie-metadata__description">
                  {context.presentationState === "loading"
                    ...
                    : context.presentationState === "unavailable"
                      ...
                      : context.presentationState === "local-only"
                        ? "No verified automatic presentation is available. Local file and group identity remains unchanged."
                        : "Presentation is descriptive only. Local file and group identity remains unchanged."}
                </Dialog.Description>
                <div className="movie-metadata__details">
...
function LibraryMovieCard({
  ...
  onLibraryDetailsUpdate: (context: LibraryDetailsContext) => void;
    ...
    movie.association === null
      ...
      : {
          presentation: {
            ...
            aspectRatio: movie.association.posterPath === null ? 0.72 : 2 / 3,
          },
          coverUrl:
    ...
    item.association == null
      ...
      : {
          presentation: {
            ...
            aspectRatio: item.association.posterPath === null ? 0.72 : 2 / 3,
          },
          coverUrl:
...
export default function App({ adultCatalogItemsFixture }: AppProps = {}) {
  ...
  const updateLibraryDetailsContext = (next: LibraryDetailsContext) => {
    setLibraryDetailsContext((current) =>
      current?.triggerId === next.triggerId ? next : current,
    );
  };
                    ...
                    <NaturalWidthBrowseGallery
                      ...
                      renderItem={(movie) => (
                        <LibraryMovieCard
                          ...
                          onLibraryDetailsUpdate={updateLibraryDetailsContext}
