...
export function parseTmdbCardCoverResponse(
  ...
  return status === "ready" &&
    ...
    Number(ratio) <= 4 &&
    source === "TMDB"
    ? { status: "ready" as const, authorityId }
    : null;
}
...
export function useTmdbCardCover(request: TmdbCardCoverRequest): TmdbCardCover {
  ...
  const [state, setState] = useState<Omit<TmdbCardCover, "retry">>({
  ...
  useEffect(() => {
    ...
    if (stableRequest.posterPath === null) {
      setState({ status: "missing", objectUrl: null, source: null, reportDecodeFailure: null });
      return () => {
        active = false;
      };
    }
    setState({ status: "loading", objectUrl: null, source: null, reportDecodeFailure: null });
    const args = requestArguments(stableRequest, requestGeneration);
    void scheduleLibraryPresentation("cover", async () => {
      ...
      const reportDecodeFailure = () => {
        ...
        objectUrl = null;
        void window.__TAURI__.core.invoke("invalidate_tmdb_card_cover", args);
        setState({ status: "unavailable", objectUrl: null, source: null, reportDecodeFailure: null });
      };
      setState({ status: "ready", objectUrl, source: "TMDB", reportDecodeFailure });
    }).catch((error: unknown) => {
      if (!active || currentKey.current !== key) return;
      setState({
        status: error instanceof Error && error.message === "missing" ? "missing" : "unavailable",
        objectUrl: null,
        source: null,
        reportDecodeFailure: null,
      });
