...
describe("automatic Library presentation", () => {
  ...
  it("keeps an open details surface truthful while presentation work resolves", async () => {
    const pendingPresentation = createDeferred<string[]>();
    const pendingCover = createDeferred<number[]>();
    savedMoviesFolder = "/Movies";
    scanMoviesMock.mockResolvedValue(["/Movies/Exact loading title.mp4"]);
    fetchLibraryPresentationMock.mockReturnValue(pendingPresentation.promise);
    fetchLibraryCoverMock.mockReturnValue(pendingCover.promise);

    render(<App />);
    selectLibrary();
    const detailsTrigger = await screen.findByRole("button", {
      name: "View Library details: Exact loading title",
    });
    expect(
      within(detailsTrigger)
        .getByLabelText("Loading presentation for Exact loading title")
        .getAttribute("data-placeholder-title"),
    ).toBe("Exact loading title");
    expect(
      within(detailsTrigger).getAllByText("Loading presentation"),
    ).toHaveLength(2);
    fireEvent.click(detailsTrigger);

    const dialog = await screen.findByRole("dialog");
    expect(within(dialog).getByText("Loading presentation")).toBeTruthy();
    expect(
      within(dialog).getByText(
        "Automatic presentation is still loading. Local file and group identity remains unchanged.",
      ),
    ).toBeTruthy();

    await act(async () => {
      pendingPresentation.resolve(automaticLibraryPresentation("movie"));
      await pendingPresentation.promise;
    });
    expect(await within(dialog).findByText("Automatic presentation")).toBeTruthy();
    expect(within(dialog).getByText("Exact provider title")).toBeTruthy();
    expect(within(detailsTrigger).getByText("Loading cover")).toBeTruthy();
  ...
  it("shows an exact-title unavailable state without claiming local-only resolution", async () => {
    savedMoviesFolder = "/Movies";
    scanMoviesMock.mockResolvedValue(["/Movies/Unavailable exact title.mp4"]);
    fetchLibraryPresentationMock.mockRejectedValue("library_enrichment_failed");

    render(<App />);
    selectLibrary();
    await screen.findByRole("button", { name: "Retry" });
    const detailsTrigger = screen.getByRole("button", {
      name: "View Library details: Unavailable exact title",
    });
    expect(
      within(detailsTrigger)
        .getByLabelText(
          "Presentation unavailable for Unavailable exact title",
        )
        .getAttribute("data-placeholder-title"),
    ).toBe("Unavailable exact title");
    expect(
      within(detailsTrigger).getAllByText("Presentation unavailable"),
    ).toHaveLength(2);
    fireEvent.click(detailsTrigger);

    const dialog = await screen.findByRole("dialog");
    expect(within(dialog).getByText("Presentation unavailable")).toBeTruthy();
    expect(within(dialog).queryByText("Local only")).toBeNull();
    expect(
      within(dialog).getByText(
        "Automatic presentation is temporarily unavailable. Local file and group identity remains unchanged.",
      ),
    ).toBeTruthy();
  });

  it("uses a retained validated ratio before image decode and preserves it after cover failure", async () => {
    const pendingCover = createDeferred<number[]>();
    savedMoviesFolder = "/Movies";
    scanMoviesMock.mockResolvedValue(["/Movies/Retained ratio.mp4"]);
    fetchLibraryPresentationMock.mockResolvedValue(
    ...
    selectLibrary();
    const card = (
      await screen.findByRole("heading", { name: "Retained ratio" })
    ).closest("article") as HTMLElement;
    await waitFor(() => expect(card.style.width).toBe("90px"));
    expect(card.querySelector("img")).toBeNull();
    expect(within(card).getByText("Loading cover")).toBeTruthy();
...
describe("native-owned FANZA Adult and VR catalogs", () => {
  ...
  it("renders Copy as the only FANZA action with local feedback and no native side effect", async () => {
    fetchFanzaCatalogMock.mockResolvedValue(
      fanzaCatalogFixture("vr", [
        {
          code: "3DSVR-1947",
    ...
    for (const action of ["Find releases", "Preview", "View details", "View on FANZA"]) {
