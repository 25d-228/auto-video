...
const automaticMetadataSources = new Set([
  "JavDB",
  "JavDB + r18.dev",
  "JavDB + JavDatabase",
...
function metadataProofMatchesRequest(
  request: LibraryPresentationRequest,
  source: string,
  providerId: string,
  ...
  if (includesJavdb) {
    return (
      verificationProvider === "JavDB" &&
      verificationProviderId === providerId &&
      /^[A-Za-z0-9]{1,64}$/.test(providerId)
    );
  }
