// javdb mobile API request signature (see docs/javdb-api.md).
//
...
//
// The middle token ("lpw6vgqzsp") and the SECRET aren't hardcoded in the app;
// they're obfuscated as base64 blobs decoded (per-byte subtract) against
// md5("30820"). We decode the blobs here instead of hardcoding the literals, so
// the values still resolve if either blob is rotated. Decoded, the middle is
// "lpw6vgqzsp" and SECRET is the 128-hex-char string below; the unit tests assert
// the decode reproduces them.
...
// obfuscated signature material. JDB_CERT5 is the decode key; JDB_BLOB_A decodes
// to the SECRET; JDB_BLOB_B decodes to the middle token.
...
// `Buffer` dependency is needed, which keeps this file clean under the DOM-only
// tsconfig.app.json.
 ...
 * for every index past its end (the `i <= last ? i : last` clamp).
 */
...
/** Decoded middle token ("lpw6vgqzsp"). */
export function javdbMiddle(): string {
  return jdbDec(JDB_CERT5, JDB_BLOB_B)
}

/**
 * Build the `jdsignature` header value: `${ts}.${middle}.${md5(`${ts}${secret}`)}`.
 *
