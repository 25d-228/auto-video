...
Auto-Video is a Tauri 2 desktop application for macOS and Windows with a React and TypeScript interface built by Vite. It scans configured Movies, TV, Adult, and VR folders and shows their volume storage usage. TV files with one conservative `S01E02` or `1x02` identity are grouped by exact show title; ambiguous files remain visible and unassociated. Adult and VR files are grouped only when one canonical product code is established safely. Adult files with ambiguous identities remain visible under their complete filename-derived titles, and exact multipart labels are shown only when one existing Part, CD, Disc, or Disk marker is unambiguous. Open and Reveal accept only exact files from the latest trusted TV, Adult, or VR scan. Auto-Video also discovers and searches TMDB Movies and TV with exact-ID details and season guides, and finds JavDB Adult and VR titles by exact product code. Exact IMDb-verified YTS Movie torrents, API Bay TV episode torrents, and product-code-verified Sukebei Adult and VR torrents can be compared, inspected as bounded torrent metainfo, and saved as `.torrent` files. After configuring the relevant category folder, explicitly selected files can be started, paused, resumed, cancelled without deleting partial data, and resumed safely after relaunch. Completed Movie, Adult, and VR transfers in their current category folder can be previewed and explicitly confirmed for safe canonical organization; TV transfers never expose organization. Organized and recoverable partial results persist across relaunch, while dismissing any terminal transfer row leaves every media file untouched. Settings provides one persisted aggregate download limit for current Movie, TV, Adult, and VR transfers, while Downloads and Dashboard summarize their shared native activity and aggregate speed. Local metadata enrichment and destructive file actions are not available. TV air schedules, playback, local-library enrichment, automatic metadata lookup, episode invention, media renaming, organization, and destructive TV actions are not available. Automatic organization, organization templates, and destructive transfer actions are not available.
...
## TV episode release comparison, inspection, and selected-file downloads
...
A manually selected row exposes **Inspect torrent**. Native code re-establishes the exact TMDB show, provider season and episode, IMDb series, API Bay item, approved category, exact release name, and v1 infohash before constructing its own metadata request. Inspection is the first operation that contacts torrent metadata sources. It allocates no media files or transfer row, rejects mismatched or unsupported metainfo, and shows the exact torrent name, infohash, size, and complete file list. **Save `.torrent`** writes the exact cached generated metainfo artifact through the native destination dialog without refetching or overwriting an existing file.

Every inspected file begins unselected. **Start download** requires at least one explicit file selection and the native-persisted TV folder. The interface sends only the current inspection identity and selected file IDs. Native code reparses the cached v1 metainfo, revalidates the complete TV and API Bay identity, selected files, canonical destination, symlink boundaries, and path conflicts, then creates storage only for selected content. Deselected files do not receive placeholders, including when they share a torrent piece with selected content.

TV transfers use the same native session, persisted aggregate limit, Downloads snapshot, and Dashboard totals as Movie, Adult, and VR transfers. Their exact show, season, episode, and provider release identity survives relaunch. Pause, resume, confirmed non-destructive cancellation, and terminal dismissal remain isolated to one row; cancellation keeps downloaded and partial data, and dismissal removes only transfer metadata. Completion appears and triggers at most one bounded TV Library scan and storage refresh only after the terminal state is durable; a persistence failure reloads as a non-running failed row without changing media. A transfer targeting an older folder cannot replace current results.

TV acquisition does not select releases or files automatically, inspect arbitrary magnets or torrent files, intentionally seed, enrich Library identity from provider data, organize or rename media, move files, delete or Trash files, cancel and delete, or clean up partial files. TV packs, multi-episode releases, specials, season zero, date-based episodes, and anime-specific numbering remain excluded.
