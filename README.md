...
## Transfer cancellation and permanent cleanup

**Cancel and keep files** is non-destructive: it stops only that transfer and retains selected files, partial data, and boundary-piece data. **Dismiss** remains metadata-only and never deletes files. A cancelled row can separately request **Delete transfer files**.

Permanent cleanup requires an accessible confirmation listing the exact selected members. Native code accepts only the transfer identity and destructive choice, establishes a bounded durable cleanup record, stops the exact transfer from writing, then revalidates every current path, fingerprint, destination boundary, and competing transfer or organization/recovery owner before deleting a regular file. Selected media and non-media files are eligible; deselected, unrelated, replaced, organized, recovery-owned, and directory paths are never deleted. Finder Trash and the Windows Recycle Bin are not used for this action.

Interrupted cleanup reloads as a non-running attention row and retries only exact remaining members. Deleted and already-absent paths are tracked separately, retained boundary-piece data is cleared only after exact success, and the row disappears only after both the primary Downloads state and cleanup recovery are durable. Complete success refreshes only the matching current Library and storage once; old-folder cleanup does not replace current results.
