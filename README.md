...
API Bay supplies the accepted infohash, not an original `.torrent` artifact. Auto-Video generates the verified metainfo container from retrieved exact-infohash metadata. **Save generated metainfo** writes only those exact cached verified bytes through the native destination dialog without refetching or overwriting. Save remains independent from Start. Trash from a transfer, permanent deletion, cancel-and-delete, and partial-file cleanup remain unavailable.

## TV episode organization

An exact durable completed TV transfer in the current TV folder exposes **Organize files** only when it contains one selected media file. Native code rebuilds the plan from the persisted show, season, episode, IMDb, API Bay, infohash, metainfo, selected-file, destination, fingerprint, current-path, and boundary-piece identity. The destination is `Exact Show/Season NN/Exact Show - SxxExx - Exact Episode.ext`, with the original extension casing preserved, and the exact proposed path must parse back to the same show, season, and episode through the TV Library contract. Transfers with multiple selected media files remain ineligible. Selected non-media files stay at their existing paths.
