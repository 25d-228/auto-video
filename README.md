...
## Movie torrent comparison, inspection, and saving
...
A manually selected row exposes **Inspect torrent** only when it contains a complete YTS URL and expected BitTorrent v1 infohash. Native code binds the exact TMDB, IMDb, YTS Movie, row, URL, and hash context before fetching the bounded artifact. Inspection verifies the infohash and shows the complete torrent name, size, and file list. **Save `.torrent`** writes the exact cached verified bytes through the native destination dialog without refetching or overwriting an existing file.

Movie inspection does not contact trackers, peers, or DHT. Auto-Video does not start Movie downloads, select Movie files for download, organize Movie releases, or mutate the Movie Library from this workflow.
