...
An exact durable completed TV transfer in the current TV folder exposes **Organize files**. Native code rebuilds the plan from the persisted show, season, episode, IMDb, API Bay, infohash, metainfo, selected-file, destination, fingerprint, current-path, and boundary-piece identity. For one selected media file, the destination is `Exact Show/Season NN/Exact Show - SxxExx - Exact Episode.ext`, with the original extension casing preserved. For multiple media files, every exact basename is retained under the same canonical show and season directories. Selected non-media files stay at their existing paths.
