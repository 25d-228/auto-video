...
## Windows cancelled transfer cleanup
...
On Windows only, an exact durably cancelled Movie, TV, Adult, or VR row can expose **Permanently clean transfer files**. After a second accessible confirmation lists every selected relative path, the renderer sends only the transfer ID. Native state revalidates the exact persisted transfer, selected media and non-media paths, fingerprints, containment, symlink and directory boundaries, and all transfer, organization, and recovery ownership before it writes cleanup authority. Deletion is bound to the validated Windows file handle and never uses the Recycle Bin. Deselected, unrelated, replaced, cross-owned, Library-only, and organization- or recovery-owned files are not deleted.

Each completed file is recorded durably so an interrupted cleanup reloads as a stopped attention row and retries only exact remaining files. One cleanup can run at a time. The row disappears only after cleanup completion and durable record removal; only a matching current category then refreshes its Library and storage. macOS exposes no permanent transfer cleanup command or action.
