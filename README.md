...
Auto-Video is a Tauri 2 desktop application for macOS and Windows with a React and TypeScript interface built by Vite. It scans configured Movies, TV, and VR folders and shows their volume storage usage. TV files with one conservative `S01E02` or `1x02` identity are grouped by exact show title; ambiguous files remain visible and unassociated. VR files are grouped by canonical product code. Open and Reveal accept only exact files from the latest trusted TV or VR scan. Auto-Video also discovers and searches TMDB Movies with exact-ID details and finds JavDB VR titles by exact product code. Verified Sukebei releases can be compared, inspected as exact torrent metainfo, and saved as `.torrent` files. After configuring a VR folder, selected files can be started, paused, resumed, cancelled without deleting partial data, and resumed safely after relaunch. Completed transfers in the current VR folder can be previewed and explicitly confirmed for safe canonical organization. Organized and recoverable partial results persist across relaunch, while dismissing their terminal rows leaves every media file untouched. Settings provides one persisted aggregate download limit for all VR transfers, while Downloads and Dashboard summarize current native transfer activity and aggregate speed. Automatic TV metadata lookup, episode invention, media renaming, and destructive TV actions are not available. Automatic VR organization, organization templates, and destructive VR actions are not available.
