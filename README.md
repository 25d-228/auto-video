...
TV transfers use the same native session, persisted aggregate limit, Downloads snapshot, and Dashboard totals as Movie, Adult, and VR transfers. Their exact show, season, episode, and provider release identity survives relaunch. Pause, resume, confirmed non-destructive cancellation, and terminal dismissal remain isolated to one row; cancellation keeps downloaded and partial data, and dismissal removes only transfer metadata. A newly completed transfer in the current TV folder triggers at most one bounded TV Library scan and storage refresh. A transfer targeting an older folder cannot replace current results.
